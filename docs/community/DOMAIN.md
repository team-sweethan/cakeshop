...
- `UNIQUE (post_id, viewer_key, viewed_on)`을 건다. **중복 방지를 애플리케이션 판단이 아니라 DB 제약이 하게 한다** — 동시 요청 두 개가 "아직 안 봤다"를 동시에 읽는 순간을 코드로는 막을 수 없다.
- 흐름은 `INSERT` 시도 → **삽입된 행이 1일 때만** `view_count` 갱신이다. 0이면 이미 오늘 센 조회이므로 아무것도 하지 않는다.
...
- Service 메서드 하나 안에서 `조회 기록 시도` → `UPDATE view_count` → `SELECT 상세` 순으로 처리하고 통째로 `@Transactional`을 건다.
