...
| **B7** | 댓글 구역 부분 로드·무한 스크롤 | 누구나 | `GET /community/{id}/comments` (조각 응답) | **완료** | 9 | `community-comment.md` |
...
| ~~무한 스크롤~~ | **댓글 구역에서 끝났다 (조각 9).** 게시글 목록은 쪽 번호 페이징을 유지한다(2026-08-19 사용자 결정). 댓글의 `이전 댓글 더 보기`는 링크로 남고 그 위에 부분 로드·스크롤 이어 붙이기가 얹혔다 — 규칙은 `specs/community-comment.md` B7이 정본이다 |
