...
- **"더 보기"를 누르면 상세를 다시 여는 것이므로 지금은 조회수가 오른다.** 이건 감수할 성질이 아니라 **고쳐야 할 것**이다 — 조회수가 순위를 정하게 된 이상(6.2 정책 변경), 댓글이 많은 글일수록 조회수가 저절로 부푸는 자가 인플레가 된다. 6.2의 중복 방지가 들어오면 같은 사람의 같은 날 재조회가 세어지지 않으므로 함께 해소된다(`PLAN.md` 조각 6, R13).
