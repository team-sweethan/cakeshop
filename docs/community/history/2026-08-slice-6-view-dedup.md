...
시드에서 **기존 버그도 하나 고쳤다.** `like_count` 재계산 UPDATE가 `updated_at`을 보존하지 않아, 좋아요를 받은 글마다 화면에 `(수정됨)`이 붙어 있었다. 6.2·6.3이 경고하는 바로 그 유형이고 같은 파일이라 함께 고쳤다. 발견 경위는 `../reviews/findings.md` 2026-08-04 행에 있다.
