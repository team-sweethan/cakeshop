...
| H31 | 재집계가 **실패하면 이전 스냅샷이 그대로 남음.** `DELETE`만 커밋되고 `INSERT`가 죽는 상태가 되지 않는다. 실행 기록도 함께 남지 않는다 | `PopularPostBatchTests` +1 — **실행 기록 없이 그 날짜의 순위 행만 심어 두고** 첫 실행에서 `INSERT`를 실패시킨다. 순위 행이 **전부 그대로이고** 실행 기록이 **생기지 않은 것**을 함께 본다. **실행 기록을 함께 심으면 안 된다** — D4가 기록 있는 날짜를 즉시 건너뛰므로 두 번째 호출이 실패 지점에 닿지도 못하고, 그러면 `@Transactional`을 떼도 이 행이 통과한다(PR #103 Codex 리뷰 4라운드). 3라운드에서 D4에 건너뛰기를 넣은 것이 이 하네스를 무력화한 자기충돌이었고, **시나리오가 다른 결정에 의존한다는 것 자체가 이 자리의 성질이다** — D4의 건너뛰기 조건을 바꾸면 이 행도 함께 봐야 한다. **H22로는 이 자리가 절대 드러나지 않는다** — 성공 실행 둘을 비교할 뿐이라 `@Transactional`이 빠지거나 프록시 경계를 벗어나도 통과한다(PR #103 Codex 리뷰). 그리고 실패했을 때의 화면은 D6 폴백 때문에 **전날 목록으로 멀쩡히 보인다** — 그날 순위가 통째로 사라진 것을 아무도 눈치채지 못한다. 자기 호출로 프록시를 우회하는 실수가 이 프로젝트에서 특히 잡기 어려운 이유다 | 계획 (조각 7) |
