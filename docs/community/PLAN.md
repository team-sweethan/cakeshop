...
| 11 | 문서 드리프트 방어 | 진행 중 (11a 완료) | 문서의 SQL 예제까지 경계 검사를 넓혔다(H36). 다음은 하네스 표↔테스트 대조와 이 문서의 분할 |
...
- **형태 검사**(`CommunityDomainBoundaryTests`) — 커뮤니티 SQL이 `members`를 건드리지 않는지, 커뮤니티 코드가 회원 도메인에서 **합의된 계약 둘만** 쓰는지. 뒤쪽은 금지 목록이 아니라 **허용 목록**이다. 금지 목록은 새로 생기는 것을 못 잡는다 — 회원 쪽에 Service가 하나 더 생기면 그것을 직접 가져다 쓰는 변경이 조용히 통과한다. 목록을 늘리는 것 자체가 "새 연동 계약을 만드는 중"이라는 신호가 되게 했다. 허용 목록이 **실제로 쓰이는 이름인지** 확인하는 검사를 하나 더 붙였다 — 참조가 하나도 없어도 첫 검사는 통과하므로, 계약 이름이 바뀌면 목록만 남고 검사는 초록불이 된다.
...
`build.gradle`에도 `src/main/java`를 `test` 입력으로 등록했다. H34는 컴파일된 클래스가 아니라 원문을 읽는데, 쓰지 않는 import를 되살리는 변경은 검사 결과를 바꾸지만 바이트코드는 그대로여서 Gradle이 `test`를 건너뛴다. H7이 `docs/`와 `src/test/java`를 등록한 것과 같은 이유다.
...
**왜 지금인가**: 조각 10을 닫고 보니 진입점(`CLAUDE.md`)이 머지된 코드보다 7개 PR만큼 뒤처져 있었고, `DOMAIN.md` 6.1의 SQL 예제는 조각 10b가 걷어낸 `JOIN members`를 그대로 들고 있었다. 밀린 것은 PR #146에서 메웠지만 **그건 재발을 막지 않는다** — 규칙("조각마다 문서를 함께 고친다")은 이미 있었고 지켜지지 않았다. R32에 적은 대로 **규칙으로 안 되는 자리**다.

**11a — 문서의 SQL 예제까지 경계 검사를 넓혔다.** `CommunityDomainBoundaryTests`가 매퍼 XML과 자바 소스만 보던 것을 `docs/community/**.md`와 진입점 `CLAUDE.md`까지 넓혔다(H36). 매퍼를 그대로 둔 채 예제에만 JOIN을 되살리면 통과하던 자리가 막혔다.

**보는 것은 마크다운 코드 블록뿐이고 산문은 보지 않는다.** 규칙을 설명하려면 금지된 이름을 입에 올려야 한다 — `DOMAIN.md` 8절의 "커뮤니티 SQL이 `members`를 JOIN해 직접 판정하지 않는다"가 바로 그것이고, 문서에 `members`가 산문으로 19번 나온다. 산문까지 보면 **규칙을 적는 행위가 규칙 위반**이 되고, 그러면 다음 사람이 지우는 것은 JOIN이 아니라 설명이다. 코드 블록 안의 SQL 주석을 걷어내는 것도 같은 이유다(PR #144에서 매퍼 쪽에 이미 배운 것이다).

**언어 표시가 없는 블록도 본다.** ```` ```sql ````만 보면 표시를 지우는 것으로 비껴갈 수 있다.

**무는지 세 방향으로 확인했다** — `DOMAIN.md` 6.1 예제에 `JOIN members`를 되돌리니 빨간불, `text` 블록에 `FROM members`를 심어도 빨간불, 예제 안에 `-- members 를 JOIN 하지 않는다`는 설명 주석을 넣었을 때는 초록불.

**11a가 막지 못하는 것**: 이 검사가 보는 것은 **낡은 참조**이지 **틀린 설명**이 아니다. "H1b가 신고 목록까지 덮는다"처럼 검사 범위를 실제와 다르게 적은 문장은 기계가 읽을 수 없고, PR #146에서 실제로 그런 것이 네 건 나왔다(전부 Codex가 잡았다). 다음 조각(하네스 표 ↔ 테스트 클래스 대조)이 그중 일부를 기계로 옮긴다.
...
| H6 | 로컬 시드를 순서대로 실행하면 카테고리 참조 데이터가 남음. 커뮤니티 시드가 `parent_comment_id`를 쓰지 않는 것도 함께. **넓혔다 (조각 7b)** — 두 시드가 `daily_popular_posts`·`popular_post_batch_runs`를 `posts`보다 **먼저** 지우는지 | `CommunitySeedTests`(+2) — 시드 파일 내용을 직접 검사한다. **삭제 순서까지 본다**: 줄이 있기만 하면 통과하도록 쓰면 `posts` 아래에 있어도 지나가는데, 그러면 FK 위반이 그대로 남는다. `post_views` 때와 같은 자리이고, 그때는 시드가 실제로 깨진 뒤에야 알았다(`e275bc7`) | **적용** (조각 1, 2026-08-05 확장) |
...
| H12 | 같은 사람의 창 안 재조회가 조회수를 올리지 않음. 새로고침·뒤로가기 전부. 창을 벗어나면 다시 셈 | `CommunityMapperTests`(+8), `CommunityViewCountTests`(6) — 실제 MariaDB로 확인한다. 창의 **폭**은 여기서 보지 않는다(H19) — 이 행의 테스트들은 창이 하루든 10분이든 똑같이 통과한다 | **적용** (조각 6, 2026-08-04 갱신) |
| H21 | 집계 SQL이 **창으로 먼저 자르는 형태**임 — `UNION ALL` + `GROUP BY post_id`이고, 게시글마다 도는 스칼라 서브쿼리가 아님 | `CommunityMapperXmlTests` +1(SQL 형태). 형태가 바뀌어도 **결과는 같아서** 결과 검증으로는 절대 안 잡힌다. 스칼라 서브쿼리로 되돌리면 대상이 창 안의 이벤트가 아니라 전체 게시글이 되는데, 데이터가 적을 때는 차이가 보이지 않는다. H1a와 같은 종류이고 방향만 반대다 | **적용** (조각 7b) |
| H22 | 같은 날짜로 **두 번 돌려도 결과가 완전히 같음** — 순위·점수·스냅샷 숫자 전부 | `PopularPostBatchTests` — 실제 MariaDB로 두 번 실행하고 행 전체를 비교한다. D4의 본체다. 동점 tiebreaker가 SQL에 없으면 여기서 깨지는데, **동점이 없는 데이터로만 확인하면 통과한다** — 같은 점수를 갖는 글을 반드시 넣는다 | **적용** (조각 7b) |
| H31 | 재집계가 **실패하면 이전 스냅샷이 그대로 남음.** `DELETE`만 커밋되고 `INSERT`가 죽는 상태가 되지 않는다. 실행 기록도 함께 남지 않는다 | `PopularPostBatchTests` +1 — **실행 기록 없이 그 날짜의 순위 행만 심어 두고** 첫 실행에서 `INSERT`를 실패시킨다. 순위 행이 **전부 그대로이고** 실행 기록이 **생기지 않은 것**을 함께 본다. **실행 기록을 함께 심으면 안 된다** — D4가 기록 있는 날짜를 즉시 건너뛰므로 두 번째 호출이 실패 지점에 닿지도 못하고, 그러면 `@Transactional`을 떼도 이 행이 통과한다(PR #103 Codex 리뷰 4라운드). 3라운드에서 D4에 건너뛰기를 넣은 것이 이 하네스를 무력화한 자기충돌이었고, **시나리오가 다른 결정에 의존한다는 것 자체가 이 자리의 성질이다** — D4의 건너뛰기 조건을 바꾸면 이 행도 함께 봐야 한다. **H22로는 이 자리가 절대 드러나지 않는다** — 성공 실행 둘을 비교할 뿐이라 `@Transactional`이 빠지거나 프록시 경계를 벗어나도 통과한다(PR #103 Codex 리뷰). 그리고 실패했을 때의 화면은 D6 폴백 때문에 **전날 목록으로 멀쩡히 보인다** — 그날 순위가 통째로 사라진 것을 아무도 눈치채지 못한다. 자기 호출로 프록시를 우회하는 실수가 이 프로젝트에서 특히 잡기 어려운 이유다 | **적용** (조각 7b) |
| H32 | **활동이 0인 날도 확정으로 기록됨** — 순위는 0건이지만 실행 기록에는 행이 남고, 그 뒤 화면이 **옛 날짜로 되돌아가지 않는다** | `PopularPostBatchTests` +1(0건인 날에도 실행 기록이 남는지), `CommunityMapperTests` +1(그 상태에서 화면이 인기글 영역을 그리지 않는지). **여기가 뚫리면 화면은 멀쩡해 보인다** — 7일 창 밖의 오래된 인기글이 어제 것인 양 무기한 걸려 있고, 순위가 안 바뀌는 것은 활동이 뜸한 날과 구분되지 않는다. D11이 존재하는 이유 그 자체라, 나중에 "실행 기록 테이블은 군더더기 아니냐"는 이야기가 나오면 이 행이 먼저 깨진다 | **적용** (조각 7b) |
| H33 | **성공한 날짜를 다시 돌려도 확정된 순위가 바뀌지 않음** — 원본이 변한 뒤에 돌려도 그대로다 | `PopularPostBatchTests` +1 — 정상 스냅샷을 만든 뒤 **좋아요를 취소하거나 댓글을 지우고** 같은 날짜로 다시 부른다. 순위·점수·근거 수치가 **전부 그대로여야** 한다. **H22로는 절대 안 잡힌다** — 그쪽은 원본을 건드리지 않고 두 번 부르므로 덮어쓰는 구현도 같은 결과를 낸다(PR #103 Codex 리뷰). 스냅샷이 스냅샷인 이유가 이 행이고, D4의 건너뛰기 판단이 실제로 살아 있는지도 여기서 본다 | **적용** (조각 7b) |
| H23 | 창의 **양쪽 경계**를 다 봄 — `targetDate - 6일 00:00:00`은 들어가고 그 **직전**(`- 6일 00:00`보다 1초라도 이른 것, 즉 `targetDate - 7일`의 이벤트)은 빠진다. 뒤쪽도 같다 — `targetDate 23:59:59`는 들어가고 `targetDate + 1일 00:00:00`은 빠진다 | `PopularPostBatchTests` +2. H19가 가르쳐 준 자리다: 한쪽만 보면 창이 1일이든 30일이든 똑같이 통과한다. 폭이 조용히 틀어지면 좁아진 쪽은 인기글이 비고 넓어진 쪽은 오래된 글이 상위를 점유하는데, 둘 다 화면으로는 "그날은 그랬나 보다"와 구분되지 않는다. **경계는 날짜가 아니라 시각으로 적는다** — "7일 전은 들어간다"처럼 날짜로 쓰면 대상일 포함 7칸인지 8칸인지가 읽는 사람마다 갈리고, 실제로 이 행이 한 번 그렇게 틀렸다(PR #103 Codex 리뷰) | **적용** (조각 7b) |
| H24 | 가중치가 실제로 25/15/1임 | `PopularPostBatchTests` +1 — 조회수만 많은 글이 좋아요 하나 있는 글을 **못 이기는 지점**을 데이터로 잡는다. 계수가 조용히 틀어지면 순위만 이상해지고 화면은 멀쩡하다. 원안의 5/3/1로 되돌아가는 것이 정확히 이 검사가 막는 실수다 | **적용** (조각 7b) |
| H30 | 댓글 점수가 **건수가 아니라 사람 수**임. 한 회원이 한 글에 댓글을 열 개 달아도 15점이고, 서로 다른 두 회원이 하나씩 달면 30점 | `PopularPostBatchTests` +1. **정상 데이터로는 두 구현이 구분되지 않는다** — 사람마다 댓글을 하나씩 다는 흔한 경우에 두 값이 같아서, 같은 회원이 여러 번 다는 데이터를 일부러 만들지 않으면 `COUNT(*)`로 되돌아가도 통과한다. 이 자리가 뚫리면 계정 하나로 TOP 20을 점유할 수 있다(D1) | **적용** (조각 7b) |
| H25 | 상태 필터가 **두 곳 모두**에 있음 — 배치 선정 시점에 이미 비노출인 글은 **스냅샷에 담기지 않고**, 담긴 뒤 차단·삭제된 글은 화면에서 빠지며 그만큼 목록이 짧아짐 | `PopularPostBatchTests` +1(선정 시점 필터 — 점수가 1위인 `DELETED` 글이 스냅샷에 **없는지**), `CommunityMapperTests` +1과 `CommunityMapperXmlTests` +1(노출 시점 필터), `CommunityScreenRenderingTests` +1(화면에서 실제로 빠지는지). **반대쪽도 함께 본다** — 정상 케이스만 보면 `status` 필터를 통째로 지운 구현이 통과한다. 조각 3·5에서 두 번 겪은 "조건이 두 벌로 갈라진 자리"와 같은 유형인데, **여기서는 두 벌인 것이 맞다** — 막는 것이 서로 다르기 때문이다(D5). 한쪽만 두면 선정에서는 지워진 글이 칸을 먹고, 노출에서는 차단된 글이 맨 위에 뜬다 | **적용** — 선정 쪽은 조각 7b, 노출 쪽은 7c |
| H26 | 좋아요 **취소**가 점수에서 빠지고, **삭제된 댓글**도 빠짐 | `PopularPostBatchTests` +2. 카운터 테이블을 두지 않은 이유(D2) 그 자체를 적어 둔 것이다. 나중에 누군가 "배치가 느리니 카운터로 미리 쌓자"고 할 때 무는 자리이고, 증가만 하는 카운터는 여기서 반드시 깨진다 | **적용** (조각 7b) |
| H27 | 확정 결과가 **하나도 없을 때** 목록 화면이 500이 아니라 인기글 영역 없이 그려짐. **확정은 됐는데 오른 글이 전부 비노출인 날**도 같은 화면이다 | `CommunityScreenRenderingTests` +2(빈 영역과 정상 렌더 양쪽), `CommunityServiceTests` +2(빈 영역이 되는 두 경로). D6. 첫 배포 직후에만 나타나는 상태라 **사람 눈으로는 한 번 지나가면 다시 볼 수 없고**, 그 한 번이 하필 첫 사용자다. 두 번째 경로는 그 뒤로도 계속 나타날 수 있는데, 날짜만 담아 돌려주면 **제목과 날짜만 있고 안은 빈** 칸이 상단에 남는다 — 사용자에게는 고장으로 보이지만 서버에는 오류가 없어 로그에도 안 남는다. 문구 부재는 낱말이 아니라 `<h2>인기글</h2>`로 본다: Thymeleaf가 HTML 주석을 응답에 그대로 내보내고 그 주석에 이 낱말이 들어 있다 | **적용** (조각 7c) |
| H28 | 목록 정렬 분기**마다** `id` tiebreaker가 있음. 정렬이 문자열 연결이 아니라 `<choose>` 분기로 갈리고, 새 분기에서도 `status` 조건이 살아 있음 | `CommunityMapperXmlTests` +2(분기마다 형태, 그리고 **두 분기가 서로 다른 SQL인지** — 한쪽만 보면 정렬을 통째로 지우고 최신순으로 고정한 구현이 통과한다), `CommunityMapperTests` +3(실제 MariaDB. 조회수와 **반대 순서의 작성 시각**을 주지 않으면 최신순 고정 구현이 그대로 통과한다), `CommunityControllerTests` +3(모르는 값·SQL 조각이 기본값으로 떨어지는지), `CommunityScreenRenderingTests` +2(필터와 정렬이 **한 링크 안에서** 서로를 잃지 않는지 — 따로 찾으면 상단 필터 링크 때문에 통과한다). H18과 같은 자리다 — 조각 5에서 분기가 늘 때 한쪽만 tiebreaker를 잃는 것을 이미 겪었다 | **적용** (조각 7a) |
| H29 | `(status, view_count, id)` 인덱스가 **그 컬럼 순서로** 존재함 | `CommunitySchemaTests` +1(`information_schema.statistics`). 없어도, 순서가 뒤집혀도 **화면 결과는 완전히 똑같고 속도만 달라진다.** 조각 4에서 겪은 "교착처럼 터지지 않고 조용히 느려지기만 하는" 자리와 같은 종류라 형태를 직접 적어 둔다 | **적용** (조각 7a) |
| H20 | 댓글 `더 보기`가 **창과 무관하게** 조회수를 올리지 않음. `?comments=`가 붙은 상세 요청이 조회수를 올리지 않는 메서드로 가고, 안 붙은 진입은 그대로 셈 | `CommunityControllerTests` +2(경로가 갈리는지, 그리고 **반대쪽도** — 한쪽만 보면 아무것도 안 세는 구현이 통과한다), `CommunityViewCountTests` +1(이력을 11분 과거로 밀어 **창이 닫힌 뒤에도** 그 메서드가 세지 않는 것을 본다 — **되돌리는 실수를 무는 것은 컨트롤러 쪽 둘이고, 이 하나는 경로가 지키는 성질 자체를 적어 둔 것이다**). 이 자리는 원래 H12에 적혀 있었는데 **H12의 테스트들은 창을 한 번도 넘지 않아 실제로는 보증하지 않았다** — 창 안에서만 확인하는 테스트는 10분 넘게 읽다 누른 `더 보기`를 통째로 놓치고, 놓친 자리는 실패가 아니라 통과로 나타난다 | **적용** (2026-08-04, PR #98 Codex 리뷰) |
| H13 | 조회수 UPDATE가 게시글 행을 **먼저** 잠그고 중복까지 판단함. 창의 기준이 DB 시계(`NOW(6)`)이고 폭이 10분임. 이력 INSERT는 중복을 삼키지 않음 | `CommunityMapperXmlTests` +2 — SQL 형태 검사. 순서를 되돌리면 **교착**이 나는데, 동시 요청이 없으면 결과가 똑같아 단일 스레드로는 드러나지 않는다. 창을 파라미터 시각으로 바꾸면 서버가 여러 대일 때만 어긋나므로 로컬에서는 절대 안 보인다 | **적용** (조각 6, 2026-08-04 갱신) |
| H14 | `posts.view_count`가 `post_views` 이력과 일치함. **동시 요청 뒤에도.** 6.2가 좋아요와 달리 증분을 택하면서 대신 약속한 것이 이것이다 | `CommunityViewCountConcurrencyTests`(3) — 8개 스레드를 배리어로 같은 순간에 출발시킨다. **이 클래스만 트랜잭션 롤백을 쓰지 않는다** — 테스트를 트랜잭션으로 감싸면 다른 스레드가 게시글을 아예 못 봐서 경쟁 자체가 일어나지 않고, 통과하지만 아무것도 검증하지 않는 테스트가 된다. **2026-08-04부터 이 클래스가 유일한 방어선이다** — 10분 창으로 바꾸면서 `uk_post_views_post_viewer_date`를 지웠고, 이제 잠금 순서가 깨져도 DB는 아무 말도 하지 않는다. 세 번째 테스트가 **창이 열린 직후**의 동시 요청을 본다(앞의 둘은 창을 한 번도 넘지 않아 창 조건을 통째로 지워도 통과한다) | **적용** (조각 6, 2026-08-04 강화) |
| H19 | 창의 폭이 실제로 10분임. **양쪽 경계를 다 본다** — 9분 전 이력은 막고, 11분 전 이력은 통과 | `CommunityMapperTests` +2(실제 MariaDB). 한쪽만 보면 창이 1분이든 하루든 똑같이 통과한다. 값이 조용히 틀어지면 좁아진 쪽은 조회수가 부풀고 넓어진 쪽은 숫자가 멈춘 것처럼 보이는데, 둘 다 화면으로는 구분되지 않는다. DB의 `NOW(6)`를 쓰므로 시각을 주입할 자리가 없어 **이력을 과거로 미는 방식**으로 확인한다 | **적용** (2026-08-04) |
| H34 | 커뮤니티 SQL이 `members`를 건드리지 않고, 커뮤니티 코드가 회원 도메인에서 **합의된 계약 둘**(`MemberCommunityQueryService`, `MemberCommunityView`)만 씀 | `CommunityDomainBoundaryTests`(3) — 주석을 걷어낸 형태 검사. JOIN만이 아니라 서브쿼리·`EXISTS` 우회도 잡고, 금지가 아니라 **허용 목록**이라 회원 쪽에 새로 생기는 것도 막힌다. 세 번째는 허용 목록이 **실제로 쓰이는 이름인지**를 본다 — 참조가 하나도 없어도 앞의 검사는 통과하므로, 계약 이름이 바뀌면 목록만 남고 초록불이 된다. JOIN을 되돌려 심어 무는 것을 확인했다 | **적용** (조각 10d) |
...
| H36 | **문서의 SQL 예제**도 회원 도메인 소유 테이블을 건드리지 않음. `docs/community/**.md`와 진입점 `CLAUDE.md` | `CommunityDomainBoundaryTests` +1 — 마크다운 코드 블록만 본다. **산문은 보지 않는다**: 규칙을 설명하려면 금지된 이름을 입에 올려야 하고(8절이 그렇다) 문서에 산문으로 19번 나온다. 산문까지 보면 **규칙을 적는 행위가 위반**이 되어, 다음 사람이 지우는 것은 JOIN이 아니라 설명이다. 블록 안의 SQL 주석을 걷어내는 것도 같은 이유다. 언어 표시가 없는 블록도 본다 — ```` ```sql ````만 보면 표시를 지워 비껴갈 수 있다. 매퍼만 보던 때는 예제에만 JOIN을 되살리면 통과했고, 조각 10b 뒤에도 6.1 예제가 실제로 그 상태로 남아 있었다(R32). 예제와 `text` 블록 양쪽에 심어 무는 것을, 설명 주석에는 안 무는 것을 확인했다 | **적용** (조각 11a) |
