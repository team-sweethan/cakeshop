...
| 7 | 조회수 정렬·인기글 | 진행 중 (7a·7b 완료, 7c 남음) | 목록 `?sort=views`, 인기글 영역, 정렬 인덱스 |
| 8 | (2차) 대댓글 | 범위 밖 | |
| 9 | (2차) 무한 스크롤 | 범위 밖 | |
...
>
> 화면 쪽(D6·D7)은 아직 여기가 정본이다 — 7c를 구현하며 6.9로 옮긴다. **7c를 이어받는 사람은 이 절을 읽고 시작한다.**

DOMAIN.md 9의 보류 항목 넷 중 **셋이 닫혔고, 남은 하나는 화면 자리다.**
...
| H25 | 상태 필터가 **두 곳 모두**에 있음 — 배치 선정 시점에 이미 비노출인 글은 **스냅샷에 담기지 않고**, 담긴 뒤 차단·삭제된 글은 화면에서 빠지며 그만큼 목록이 짧아짐 | `PopularPostBatchTests` +1(선정 시점 필터 — 점수가 1위인 `DELETED` 글이 스냅샷에 **없는지**), `CommunityMapperTests` +2(노출 시점 필터). **반대쪽도 함께 본다** — 정상 케이스만 보면 `status` 필터를 통째로 지운 구현이 통과한다. 조각 3·5에서 두 번 겪은 "조건이 두 벌로 갈라진 자리"와 같은 유형인데, **여기서는 두 벌인 것이 맞다** — 막는 것이 서로 다르기 때문이다(D5). 한쪽만 두면 선정에서는 지워진 글이 칸을 먹고, 노출에서는 차단된 글이 맨 위에 뜬다 | **부분 적용** — 선정 쪽은 조각 7b(`PopularPostBatchTests`), 노출 쪽은 7c에서 |
