...
| D5 | 배치는 **TOP 20 저장**, 화면은 **10건 노출**. 노출 시 현재 `posts.status = 'PUBLISHED'`를 다시 확인 | 선정된 뒤 삭제·차단될 수 있다. 노출 판단의 유일 기준은 언제나 현재 `status`다(4.1). 20−10의 여유가 그만큼을 흡수한다 |
| D6 | 화면은 `MAX(ranking_date)` 하나를 읽는다. 확정 결과가 하나도 없으면 **인기글 영역 자체를 그리지 않는다** | 첫 배포 후 첫 배치 전에는 보여 줄 것이 없고, 배치를 한 번 거른 날에도 없다. 최신 확정일로 폴백하면 거른 밤이 "빈 화면"이 아니라 "어제 목록 유지"로 degrade 된다 |
...
**댓글 작성 자체를 제한하지는 않는다.** 한 글에 여러 번 답하는 것은 정상적인 대화이고, 막으면 커뮤니티의 기능이 줄어든다. 고칠 자리는 쓰기 규칙이 아니라 **점수가 무엇을 신뢰하는가**다. 화면의 `댓글 N`은 지금처럼 건수 그대로다(4.4) — 점수와 표시가 다른 것을 세는 셈이지만, 둘은 목적이 다르다. 표시는 "얼마나 이야기가 오갔나"이고 점수는 "몇 사람이 반응했나"다.
...
```sql
ALTER TABLE post_views ADD INDEX ix_post_views_created (created_at, post_id);
ALTER TABLE post_likes ADD INDEX ix_post_likes_created (created_at, post_id);
ALTER TABLE comments   ADD INDEX ix_comments_created   (created_at, post_id);
```
...
| H25 | 스냅샷에 담긴 뒤 **차단·삭제된 글이 화면에서 빠지고**, 그만큼 목록이 짧아짐 | `CommunityMapperTests` +2. **반대쪽도 함께 본다** — 정상 케이스만 보면 `status` 필터를 통째로 지운 구현이 통과한다. 조각 3·5에서 두 번 겪은 "조건이 두 벌로 갈라진 자리"와 같은 유형이고, 여기서 새면 차단된 글이 목록 맨 위에 뜬다 | 계획 (조각 7) |
...
| R23 | **배치가 실패한 날을 손으로 다시 돌릴 방법이 없다.** 수동 실행 경로(관리자 화면·HTTP 엔드포인트)를 1차에 넣지 않았다 — 인증·권한·화면 명세가 함께 붙는데 아직 한 번도 실패한 적이 없다. D6의 폴백 덕에 **화면은 최신 확정일 목록으로 버티므로 손해가 겉으로 드러나지도 않는다.** 대신 그날의 순위는 영원히 생기지 않고, 다음 날 배치는 자기 날짜만 만든다. **되돌아올 계기**: 실제로 거르는 일이 생기면 관리자 수동 실행을 넣는다. 그때 임의 날짜를 받게 되므로 미래 날짜·너무 오래된 날짜를 막는 판단이 함께 필요하다 | 1차에선 수용 (2026-08-04) |
