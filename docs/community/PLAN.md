...
| H12 | 같은 사람의 창 안 재조회가 조회수를 올리지 않음. 새로고침·뒤로가기·댓글 `더 보기` 전부. 창을 벗어나면 다시 셈 | `CommunityMapperTests`(+8), `CommunityViewCountTests`(5) — 실제 MariaDB로 확인한다. 창의 **폭**은 여기서 보지 않는다(H19) — 이 행의 테스트들은 창이 하루든 10분이든 똑같이 통과한다 | **적용** (조각 6, 2026-08-04 갱신) |
