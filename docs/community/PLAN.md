...
| H1c | 조회수 증가가 게시글을 "수정됨"으로 만들지 않음 | `CommunityViewCountTests.getPostDetail_doesNotMarkPostAsEdited` | **적용** (조각 1) |
...
| H34 | 커뮤니티 SQL이 `members`를 건드리지 않고, 커뮤니티 코드가 회원 도메인에서 **합의된 계약 둘**(`MemberCommunityQueryService`, `MemberCommunityView`)만 씀 | `CommunityDomainBoundaryTests`(2) — SQL의 회원 소유 테이블 참조를 막고, 자바 참조가 허용 계약과 정확히 일치하는지 확인한다 | **적용** (조각 10d) |
...
| H36 | **이 표가 실제 테스트와 어긋나지 않음.** 양방향 — `적용` 행이 가리키는 클래스·메서드가 실존하고, 커뮤니티 테스트 클래스가 전부 이 표 어딘가에 적혀 있음 | `CommunityHarnessDocTests`(2) — PLAN.md를 파싱해 대조한다. **정방향만 두면 표가 비는 것을 못 잡는다** — 하네스급 테스트를 새로 쓰고 안 올려도 표의 이름은 전부 실재하므로 초록불이다. 실제로 이 검사를 처음 돌렸을 때 정방향은 0건이고 어긋남은 역방향 4건이 전부였다. `적용`이 아닌 행은 가리킬 테스트가 없는 것이 정상이라 정방향에서 뺀다. 회원 폴더의 `MemberCommunity*`는 남의 영역이라 역방향 범위 밖이다 | **적용** (2026-08-07) |
...
조각을 끝낼 때 **그 조각이 추가한 하네스를 여기 올리고 조각 표의 상태를 바꾼다.** 이 문서가 정본이므로, 여기가 현실과 어긋나면 다음 작업자가 끝난 일을 다시 한다. **H36이 그 규칙을 이제 강제한다** — 올리는 것을 잊으면 다음 `gradlew test`가 빨간불이다.
...
>
> **닫힌 PR #147이 문서의 SQL 예제 경계 검사를 H36으로 부른다.** 그 브랜치는 낡은 베이스라 닫혔고 `dev`에 들어온 적이 없으므로, 여기서는 H36을 이 표 대조 검사에 쓴다. 그 검사를 되살릴 때는 현재 `dev` 위에 새로 얹으면서 번호를 다시 받는다 — 들어오지도 않은 번호를 비워 두면 다음 사람은 빠진 자리를 보고 무엇을 잃었는지부터 찾는다.
