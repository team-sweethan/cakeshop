...
H7은 리뷰를 거치며 두 번 강해졌다. 처음에는 소스에 메서드 이름이 있는지만 봤고(꺼진 테스트가 통과), 다음에는 실행되는 테스트인지까지 봤다(**엉뚱한 테스트를 연결해도 통과**). 지금은 그 테스트 본문에 문구가 실제로 등장하는지까지 본다. 세 번 모두 "검사가 있다"와 "검사가 문다"가 다르다는 같은 실수였다.
...
| H7 | 화면 명세 `SCREENS.md`가 실제 화면과 어긋나지 않음. 고객·관리자 템플릿 6종 전부. 문서가 "이 테스트가 지킨다"고 적은 테스트가 **실행되며 그 문구를 실제로 확인하는지**, `계획` 화면이 아직 안 만들어졌는지도 함께 | `CommunityScreenDocTests`(6) — 문서를 파싱해 템플릿 파일·문구와 대조하고, 참조된 테스트는 리플렉션으로 실행 여부를, 소스 본문으로 문구 확인 여부를 본다. `build.gradle`에서 `docs/`를 `test` 입력으로 등록해야 문서만 고쳐도 다시 돈다 | **적용** (조각 1) |
