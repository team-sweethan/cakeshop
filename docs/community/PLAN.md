...
- `CommunityHomeQueryService`에 공지 조회 메서드 추가(3건). **새 계약 클래스를 만들지 않는다**
- `HomeService`·`HomeController`·`main.html`
- `home`은 공통 협의 도메인이라 PR에서 확인을 받는다(조각 13과 같다)

**미정**: 메인에서 공지를 **어느 자리에** 둘지. 커뮤니티 목록에서는 공지가 인기글 위인데, 메인은
`추천·인기 상품`과 `매장 정보` 사이에 인기글이 있다. 같은 자리에서 인기글 위로 붙일지, 메인 최상단
으로 뺄지가 갈린다. 착수 전에 정한다.

**검증**: 3건, 그릴 것이 없으면 영역이 통째로 빠짐, 실제 렌더링.
