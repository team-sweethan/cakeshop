...
| H18 | 창의 폭이 실제로 10분임. **양쪽 경계를 다 본다** — 9분 전 이력은 막고, 11분 전 이력은 통과 | `CommunityMapperTests` +2(실제 MariaDB). 한쪽만 보면 창이 1분이든 하루든 똑같이 통과한다. 값이 조용히 틀어지면 좁아진 쪽은 조회수가 부풀고 넓어진 쪽은 숫자가 멈춘 것처럼 보이는데, 둘 다 화면으로는 구분되지 않는다. DB의 `NOW(6)`를 쓰므로 시각을 주입할 자리가 없어 **이력을 과거로 미는 방식**으로 확인한다 | **적용** (2026-08-04) |
