...
> ⚠️ **정본은 아직 이 파일이다.** 아래 D1~D10은 조각 7의 **계획**이고, `DOMAIN.md`에는 인기글 절(6.9)이 **아직 없다.** 7b·7c를 구현하며 6.9를 신설하고 그때 정본을 옮긴다 — 구현 전에 옮기면 고쳐질 값을 정본에 적게 된다. **7b를 이어받는 사람은 이 절을 읽고 시작한다.**

DOMAIN.md 9의 보류 항목 넷 중 **지금까지 닫힌 것은 하나뿐이다.**

| 보류 항목 | 상태 |
|---|---|
| `sort=likes` 추가 여부 | **닫음 (조각 7a)**. 넣지 않는다 — DOMAIN.md 6.1에 근거와 함께 반영 완료 |
| 인기글의 기간 | D1로 정했으나 **9절은 아직 열려 있다.** 7b에서 6.9와 함께 닫는다 |
| 인기글을 어디에 두나 | D7로 정했으나 **9절은 아직 열려 있다.** 7c에서 6.9와 함께 닫는다 |
| `post_views` 보관 기간 | D9로 정했으나 **9절은 아직 열려 있다.** 7b에서 6.9와 함께 닫고 R25로 옮긴다 |
...
| D4 | 멱등성 = **한 트랜잭션에서 `DELETE by date` → `INSERT` → 실행 기록**. 단 **이미 기록된 날짜는 아무것도 하지 않고 끝낸다** | 스케줄러는 실패하고 다시 돌 수 있다. 같은 입력이면 같은 결과여야 하므로 동점 tiebreaker까지 SQL에 박는다. **그런데 원본이 변하면 입력이 같지 않다** — 좋아요 취소·댓글 삭제 뒤에 같은 날짜를 다시 돌리면 확정된 순위와 근거 수치가 조용히 바뀌고, 그러면 "원본이 변해도 그날 기록은 남는다"는 스냅샷의 목적이 무너진다(PR #103 Codex 리뷰). 그래서 재실행은 **실패한 날짜에만** 의미가 있다. 성공한 날짜를 건너뛰어도 잃는 것이 없는 이유는 H31이 "실패하면 아예 손대지 않음"을 보증하기 때문이다 — **기록이 있다는 것이 곧 성공했다는 뜻**이라 판단이 성립한다. 덤으로 R22의 낭비도 준다: 인스턴스가 여럿이어도 먼저 기록한 하나만 일하고 나머지는 즉시 끝난다 |
...
1. **JDBC 세션 시간대를 고정한다** (`connectionTimeZone=Asia/Seoul&forceConnectionTimeZoneToSession=true`). 한 줄이고 전역이라, 앞으로 시각을 다루는 모든 경로가 같은 기준을 갖는다.
...
**경계 테스트는 시간대까지 본다**(H23). DB를 UTC로 띄운 컨테이너에서도 서울 기준 창이 나오는지 확인하지 않으면, 로컬이 우연히 서울이라 통과하고 운영에서만 어긋난다 — H19가 "로컬에서는 절대 안 보인다"고 적어 둔 것과 같은 종류다.
...
```sql
CREATE TABLE daily_popular_posts (
...
- `PopularPostBatchService.createDailyRanking(LocalDate)` — `@Transactional`. **실행 기록을 먼저 확인하고 있으면 즉시 끝낸다**(D4) → `DELETE` → `INSERT ... SELECT` → **실행 기록 `INSERT`**. 넷이 한 트랜잭션인 것이 중요하다: 순위만 들어가고 기록이 없으면 다음 실행이 그 날짜를 다시 계산하고, 기록만 들어가고 순위가 없으면 0건인 날과 구분되지 않는다
...
| H6 | 로컬 시드를 순서대로 실행하면 카테고리 참조 데이터가 남음. 커뮤니티 시드가 `parent_comment_id`를 쓰지 않는 것도 함께. **조각 7에서 넓힌다** — 두 시드가 `daily_popular_posts`·`popular_post_batch_runs`를 `posts`보다 **먼저** 지우는지 | `CommunitySeedTests`(+2) — 시드 파일 내용을 직접 검사한다. **삭제 순서까지 본다**: 줄이 있기만 하면 통과하도록 쓰면 `posts` 아래에 있어도 지나가는데, 그러면 FK 위반이 그대로 남는다. `post_views` 때와 같은 자리이고, 그때는 시드가 실제로 깨진 뒤에야 알았다(`e275bc7`) | **적용** (조각 1), 조각 7에서 확장 |
...
| H21 | 집계 SQL이 **창으로 먼저 자르는 형태**임 — `UNION ALL` + `GROUP BY post_id`이고, 게시글마다 도는 스칼라 서브쿼리가 아님 | `CommunityMapperXmlTests` +1(SQL 형태). 형태가 바뀌어도 **결과는 같아서** 결과 검증으로는 절대 안 잡힌다. 스칼라 서브쿼리로 되돌리면 대상이 창 안의 이벤트가 아니라 전체 게시글이 되는데, 데이터가 적을 때는 차이가 보이지 않는다. H1a와 같은 종류이고 방향만 반대다 | 계획 (조각 7) |
| H22 | 같은 날짜로 **두 번 돌려도 결과가 완전히 같음** — 순위·점수·스냅샷 숫자 전부 | `PopularPostBatchTests` — 실제 MariaDB로 두 번 실행하고 행 전체를 비교한다. D4의 본체다. 동점 tiebreaker가 SQL에 없으면 여기서 깨지는데, **동점이 없는 데이터로만 확인하면 통과한다** — 같은 점수를 갖는 글을 반드시 넣는다 | 계획 (조각 7) |
| H31 | 재집계가 **실패하면 이전 스냅샷이 그대로 남음.** `DELETE`만 커밋되고 `INSERT`가 죽는 상태가 되지 않는다 | `PopularPostBatchTests` +1 — 정상 스냅샷을 만든 뒤 `INSERT`를 실패시키고, 그 날짜의 행이 **전부 그대로인지** 본다. **H22로는 이 자리가 절대 드러나지 않는다** — 성공 실행 둘을 비교할 뿐이라 `@Transactional`이 빠지거나 프록시 경계를 벗어나도 통과한다(PR #103 Codex 리뷰). 그리고 실패했을 때의 화면은 D6 폴백 때문에 **전날 목록으로 멀쩡히 보인다** — 그날 순위가 통째로 사라진 것을 아무도 눈치채지 못한다. 자기 호출로 프록시를 우회하는 실수가 이 프로젝트에서 특히 잡기 어려운 이유다 | 계획 (조각 7) |
| H32 | **활동이 0인 날도 확정으로 기록됨** — 순위는 0건이지만 실행 기록에는 행이 남고, 그 뒤 화면이 **옛 날짜로 되돌아가지 않는다** | `PopularPostBatchTests` +1(0건인 날에도 실행 기록이 남는지), `CommunityMapperTests` +1(그 상태에서 화면이 인기글 영역을 그리지 않는지). **여기가 뚫리면 화면은 멀쩡해 보인다** — 7일 창 밖의 오래된 인기글이 어제 것인 양 무기한 걸려 있고, 순위가 안 바뀌는 것은 활동이 뜸한 날과 구분되지 않는다. D11이 존재하는 이유 그 자체라, 나중에 "실행 기록 테이블은 군더더기 아니냐"는 이야기가 나오면 이 행이 먼저 깨진다 | 계획 (조각 7) |
| H33 | **성공한 날짜를 다시 돌려도 확정된 순위가 바뀌지 않음** — 원본이 변한 뒤에 돌려도 그대로다 | `PopularPostBatchTests` +1 — 정상 스냅샷을 만든 뒤 **좋아요를 취소하거나 댓글을 지우고** 같은 날짜로 다시 부른다. 순위·점수·근거 수치가 **전부 그대로여야** 한다. **H22로는 절대 안 잡힌다** — 그쪽은 원본을 건드리지 않고 두 번 부르므로 덮어쓰는 구현도 같은 결과를 낸다(PR #103 Codex 리뷰). 스냅샷이 스냅샷인 이유가 이 행이고, D4의 건너뛰기 판단이 실제로 살아 있는지도 여기서 본다 | 계획 (조각 7) |
| H23 | 창의 **양쪽 경계**를 다 봄 — `targetDate - 6일 00:00:00`은 들어가고 그 **직전**(`- 6일 00:00`보다 1초라도 이른 것, 즉 `targetDate - 7일`의 이벤트)은 빠진다. 뒤쪽도 같다 — `targetDate 23:59:59`는 들어가고 `targetDate + 1일 00:00:00`은 빠진다 | `PopularPostBatchTests` +2. H19가 가르쳐 준 자리다: 한쪽만 보면 창이 1일이든 30일이든 똑같이 통과한다. 폭이 조용히 틀어지면 좁아진 쪽은 인기글이 비고 넓어진 쪽은 오래된 글이 상위를 점유하는데, 둘 다 화면으로는 "그날은 그랬나 보다"와 구분되지 않는다. **경계는 날짜가 아니라 시각으로 적는다** — "7일 전은 들어간다"처럼 날짜로 쓰면 대상일 포함 7칸인지 8칸인지가 읽는 사람마다 갈리고, 실제로 이 행이 한 번 그렇게 틀렸다(PR #103 Codex 리뷰) | 계획 (조각 7) |
| H24 | 가중치가 실제로 25/15/1임 | `PopularPostBatchTests` +1 — 조회수만 많은 글이 좋아요 하나 있는 글을 **못 이기는 지점**을 데이터로 잡는다. 계수가 조용히 틀어지면 순위만 이상해지고 화면은 멀쩡하다. 원안의 5/3/1로 되돌아가는 것이 정확히 이 검사가 막는 실수다 | 계획 (조각 7) |
| H30 | 댓글 점수가 **건수가 아니라 사람 수**임. 한 회원이 한 글에 댓글을 열 개 달아도 15점이고, 서로 다른 두 회원이 하나씩 달면 30점 | `PopularPostBatchTests` +1. **정상 데이터로는 두 구현이 구분되지 않는다** — 사람마다 댓글을 하나씩 다는 흔한 경우에 두 값이 같아서, 같은 회원이 여러 번 다는 데이터를 일부러 만들지 않으면 `COUNT(*)`로 되돌아가도 통과한다. 이 자리가 뚫리면 계정 하나로 TOP 20을 점유할 수 있다(D1) | 계획 (조각 7) |
| H25 | 상태 필터가 **두 곳 모두**에 있음 — 배치 선정 시점에 이미 비노출인 글은 **스냅샷에 담기지 않고**, 담긴 뒤 차단·삭제된 글은 화면에서 빠지며 그만큼 목록이 짧아짐 | `PopularPostBatchTests` +1(선정 시점 필터 — 점수가 1위인 `DELETED` 글이 스냅샷에 **없는지**), `CommunityMapperTests` +2(노출 시점 필터). **반대쪽도 함께 본다** — 정상 케이스만 보면 `status` 필터를 통째로 지운 구현이 통과한다. 조각 3·5에서 두 번 겪은 "조건이 두 벌로 갈라진 자리"와 같은 유형인데, **여기서는 두 벌인 것이 맞다** — 막는 것이 서로 다르기 때문이다(D5). 한쪽만 두면 선정에서는 지워진 글이 칸을 먹고, 노출에서는 차단된 글이 맨 위에 뜬다 | 계획 (조각 7) |
| H26 | 좋아요 **취소**가 점수에서 빠지고, **삭제된 댓글**도 빠짐 | `PopularPostBatchTests` +2. 카운터 테이블을 두지 않은 이유(D2) 그 자체를 적어 둔 것이다. 나중에 누군가 "배치가 느리니 카운터로 미리 쌓자"고 할 때 무는 자리이고, 증가만 하는 카운터는 여기서 반드시 깨진다 | 계획 (조각 7) |
| H27 | 확정 결과가 **하나도 없을 때** 목록 화면이 500이 아니라 인기글 영역 없이 그려짐 | `CommunityScreenRenderingTests` +1. D6. 첫 배포 직후에만 나타나는 상태라 **사람 눈으로는 한 번 지나가면 다시 볼 수 없고**, 그 한 번이 하필 첫 사용자다 | 계획 (조각 7) |
