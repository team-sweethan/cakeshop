...
**확인을 언제 받느냐는 남의 코드가 깨지는지로 가른다.**

| 하는 일 | 확인 시점 |
|---|---|
| 소유 도메인에 **연동 계약을 새 파일로 추가** | **먼저 만들고** 담당자를 리뷰어로 지정해 PR에서 확인받는다 |
| 기존 **공개 계약의 시그니처·동작 변경** | **구현 전에 합의한다** |
| 다른 도메인의 데이터를 바꾸는 **쓰기 계약** | **구현 전에 합의한다** (잠금 순서는 15.7) |

새 파일 추가는 남의 호출부를 깨뜨리지 않는다. 조건은 **담당자의 기존 파일을 수정하지 않는 것** 하나다 — `OrderMapper`에 메서드를 얹지 말고 `OrderReviewMapper`를 새로 만든다. 사전 합의를 새 파일까지 필수로 걸면 6명이 서로의 답을 기다리며 멈추는데, 그것이 막는 위험은 시그니처를 다시 고치는 것뿐이라 값이 맞지 않는다(2026-08-08 팀 합의).

반대로 이미 있는 계약을 바꾸면 **남의 호출부가 함께 깨진다.** 쓰기 계약은 잠금 순서가 어긋나도 단일 요청에서는 결과가 같아 **테스트가 통과한다**(15.7). 이 둘은 먼저 합의한다.

- **사전 합의가 필요한 쪽인데 상대 구현이 아직 없으면, 시그니처만 합의하고 쓰는 쪽은 stub으로 개발을 진행한다.** 상대 도메인의 구현을 기다리며 멈추지 않는다.
