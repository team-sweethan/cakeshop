...
| `orders.status` | 주환 | 결제대기/확인중/픽업대기/픽업완료/취소/반려/만료 | **`OrderStatus` 7개 (확정)** | ✅ 코드 확정 |
...
**`OrderStatus` (주문·주문제작 공통, 7개 + 전이규칙 — 코드에 확정됨)**
...
허용된 상태 → CANCELED                 (취소, 최종)
최종 상태: REJECTED / PICKED_UP / CANCELED / EXPIRED
```
전이 규칙은 `OrderStatus.canTransitionTo()`가 소유한다. SQL의 `CHECK`는 7개 값 집합만 나열한다.
