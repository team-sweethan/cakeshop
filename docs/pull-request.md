...
- migration 파일명은 직접 짓지 않는다. `gradlew newMigration -Pdesc=<snake_case>`로 초안을 만들고, SQL을 다 쓴 뒤 `gradlew promoteMigration -Pdesc=<snake_case>`로 승격한다.
- 공통 schema migration은 `db/migration`, 로컬 seed는 `db/seed`에 둔다. `rds` 프로필은 Flyway를 비활성화하며, RDS 변경은 별도 검토·승인 절차로 반영한다.
