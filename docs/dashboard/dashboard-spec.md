...
| 목록 필드 | 확정 | 픽업 시간, 주문번호, 대표 상품명, 긴급도, 상세 링크용 주문 ID |
| 상품명 | 확정 | 주문 시점 상품 스냅샷을 사용하고 여러 상품이면 `대표 상품명 외 N개`로 표시 |
| 지난 픽업 시각 | 확정 | 오늘 아직 픽업되지 않은 주문은 예정 시각이 지나도 유지하고 먼저 표시 |
| 긴급도 기준 시각 | 확정 | 대시보드 요청에서 생성한 `DashboardView.queryReferenceAt` 사용 |
| 지연 | 확정 | `pickup_at`이 조회 기준 시각보다 이전이면 `지연`으로 표시 |
| 임박 | 확정 | `pickup_at`이 조회 기준 시각 이상이고 정확히 1시간 후까지이면 `임박`으로 표시 |
| 예정 | 확정 | `pickup_at`이 조회 기준 시각에서 1시간을 초과해 남았으면 `예정`으로 표시 |
| 긴급도 배지 | 확정 | `지연`은 위험, `임박`은 경고, `예정`은 성공 배지 사용 |
...
결제 테이블을 별도로 조회하지 않는다. 일정 목록은 모든 행에서 동일한 주문 상태 대신 조회 기준
시각과 `pickup_at`을 비교한 긴급도를 표시하며, 기존 픽업 시각 오름차순 정렬은 유지한다.
...
| 게시글 상태 | 확정 | 관리자가 신고를 처리할 수 없는 `PostStatus.DELETED` 제외 |
...
제외한다. 게시글을 차단하거나 신고를 기각할 때 해당 게시글의 미처리 신고를 함께 처리하므로 신고
건수가 아닌 게시글 수를 집계한다.
...
- 원본 테이블을 직접 조회하는 것은 요청 시점의 최신 데이터 조회를 의미하며, 화면이 자동으로
  갱신된다는 의미는 아니다.
- 조회 Service는 한 번의 요청에서 `Clock`으로 조회 기준 시각을 한 번만 생성한다.
- 조회 기준 시각은 오늘의 시작·종료 범위를 계산하고 `DashboardView`에 전달하는 데 함께 사용한다.
- 조회 기준 시각은 `Asia/Seoul` 기준으로 화면에 `yyyy.MM.dd HH:mm:ss` 형식으로 표시한다.
- 화면에는 조회가 끝난 시각이 아니라 해당 요청의 지표 조회에 사용한 기준 시각임을 나타내도록
  `조회 기준`이라는 이름을 사용한다.
...
- 대시보드 화면의 조회 기준 시각 옆에 `새로고침` 버튼을 표시한다.
- 새로고침 버튼은 `GET /admin`으로 이동하여 모든 대시보드 지표와 목록을 다시 조회한다.
- 수동 새로고침은 별도의 상태 변경 요청이나 사용자 입력값을 전송하지 않는다.
