...
| 특정 회원 | `SPECIFIC_MEMBERS` | 양의 정수 필수 | 상세 화면에서 관리자가 선택해 시작 전에도 수동 발급 |
...
- 발급 INSERT에서는 쿠폰의 현재 관리 상태·만료 여부·수량·중복 발급 이력을 항상 다시 검증한다. 시작 전 사전 발급을 허용한 정책만 시작 일시 조건을 건너뛰며, 실제 쿠폰 사용 가능 여부는 별도의 주문 조회·예약 조건에서 `startsAt` 이후인지 확인한다. 따라서 대상 조회 후 쿠폰이 중지·만료되거나 이미 발급된 경우에는 새 발급이 생성되지 않는다.
...
- 특정 회원 수동 발급은 시작 전에도 가능하다. 단, 발급 허용 상태이며 만료 전이어야 하고, 발급받은 회원은 `startsAt` 이후에만 쿠폰을 사용할 수 있다.
...
쿠폰 SQL은 쿠폰 정책·만료 여부·수량·중복 발급 여부를 검증하며, 사전 발급 허용 여부에 따라 시작 일시 조건을 적용한다.
