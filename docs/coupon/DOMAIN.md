...
2. 발급 중지 쿠폰: 최근 수정순
3. 발급 예정 쿠폰: 시작 임박순
4. 수량 소진 쿠폰: 마지막 발급으로 변경된 시각(`updated_at`)의 최신순
5. 종료 쿠폰: 최근 종료순

수량 소진은 `SPECIFIC_MEMBERS` 쿠폰에서만 발생하며, 자동 발급 대상은 발급 수량 제한이 없어 이 그룹에 포함되지 않는다.

### 2.5 관리자 목록 검색

관리자 목록은 쿠폰명·표시 상태·할인 유형·사용 기간으로 검색한다.
