...
| --- | --- | --- | --- | --- |
| `chore/branch-status-workflow` | 2026-07-29 | HyunGyu-Cho | f91c2a1 | fix: 브랜치 현황 표의 이스케이프와 필드 분리 보강 |
...
| `chore/branch-status-workflow` | 4 | 0 | 작업 중 |
| `main` | 0 | 62 | 릴리스 브랜치 |
...
- 위 수치는 기준 커밋 `237b0d2` 시점의 스냅샷이다. 실제 값은 이 문서가 머지되는 순간 이미 더 커져 있다. 이 문서 자신의 커밋과 merge commit 이 `dev`에 얹히고(1~2), 그 사이 다른 PR 이 먼저 머지됐다면 그 커밋 수만큼 더 더해진다. 정확한 값은 `git rev-list --left-right --count origin/dev...origin/<브랜치>`로 확인한다.
