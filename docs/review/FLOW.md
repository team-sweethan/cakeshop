...
> 이 문서는 Review 도메인의 **전체 흐름을 잡기 위한 초안**이다. 세부 규칙은 아직 정하지 않았다.
...
흐름의 방향은 정해졌고, 각 지점의 규칙이 남았다.

| 지점 | 미정 |
|---|---|
| 3.2 작성 | 평점 4종의 정의 — 스키마(`overall`/`taste`/`design`/`service`)와 목업(`맛`/`디자인`/`포장`/`응대`)이 어긋난다 |
| 3.2 작성 | 수정·삭제를 여는지, 작성 기간 제한이 있는지 |
| 3.3 집계 | 4개 평점 중 무엇이 `average_rating`인지, 증분인지 재계산인지 |
| 3.4 노출 | 비로그인에게 여는지 (`/reviews/**`는 현재 인증 필요, `/products/**`는 공개) |
| 3.5 관리자 | 숨김만인지 삭제도인지 |
| 3.6 알림 | 답글 외에 알릴 시점이 있는지 |
