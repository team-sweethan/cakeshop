...
**이 자리를 기계로 막는 검사는 없다.** 한때 문서 형태 하네스(H1~H6)를 세웠지만 인벤토리의 `현재` 열 같은 내용 드리프트는 원리상 잡지 못했고, 2026-08-09에 하네스 전체를 걷어냈다(`../PLAN.md` 결정 로그). **없는 안전망을 있다고 적는 것이 없는 것보다 나쁘다** — 다음 사람이 그것을 믿고 안 본다. 문서와 코드의 일치는 조각 완료 시 문서 동기화(`domain/review/CLAUDE.md`)와 사람 리뷰가 맡는다.
