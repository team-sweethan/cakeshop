...
- 지금 이 예외로 허용되는 것은 D1 하나다 — `ProductReviewMapper`가 `reviews`를 세어 `products.average_rating`·`review_count`를 유지한다. **자기 컬럼을 자기가 계산하는 것이라 도메인 침범이 아니라 구체화 뷰 유지에 가깝다.**
- **기준은 "전용 매퍼냐"가 아니라 "무엇을 위해 읽느냐"다.** 파일을 하나 더 만들면 통과하는 규칙이면 그건 규칙이 아니라 절차다. 전용 매퍼는 허용의 근거가 아니라 허용된 읽기를 한 곳에 모아 두는 방법이다.
- 이 선으로 실제 문제를 일으켰던 것들은 그대로 막힌다.
...
- **예외를 열어도 결합이 사라지지는 않는다.** `reviews.status` 어휘가 바뀌면 `ProductReviewMapper`가 조용히 어긋나고 컴파일도 테스트도 통과한다. 이건 규칙이 아니라 테스트로 막는다 — 집계 테스트에 **"`BLOCKED` 후기는 평균에서 빠진다"**를 넣어 어휘가 바뀌면 빨간불이 뜨게 한다.
