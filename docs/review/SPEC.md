...
> - 위 SQL은 **계약의 형태를 정하기 위한 것**이지 리뷰 매퍼에 넣을 문장이 아니다. `NOT EXISTS`의 `reviews` 조회만 리뷰 쪽에 남는다.
...
- **순서가 정해져 있다: 상품 행 잠금 → `reviews` INSERT → 집계.** 셋 다 같은 트랜잭션이다.
  ...
  - **순서만으로는 부족하다.** 위 검증 4가지가 평범한 SELECT라 `REPEATABLE READ` 읽기 스냅샷이 잠금보다 먼저 굳는다. 3번 집계가 잠금 읽기여야 하는 이유이며, 계약은 D1에 있다.
...
| 항목 | 결정 시점 |
|---|---|
| **주문 도메인 계약 시그니처**(A1 목록 + A2·A3 단건 검증) | **조각 1 착수 전, 주환님과 합의** (A1) |
...
**2026-08-06 — 주문 데이터도 공개 계약으로 받는다(PR #121 3차 리뷰 P1).** A1의 SQL이 `order_items`와 `orders`를 직접 JOIN하고 A2·A3의 검증도 같은 접근을 전제하고 있었다. D1에서 상품 쪽에 세운 기준(15절)을 **주문 쪽에는 적용하지 않은 자리**였다. 계약을 여기서 혼자 정하지 않고 조각 1 착수 전 주환님과 합의할 항목으로 9절에 올린다 — 15절이 "시그니처만 먼저 합의하고 stub으로 진행"을 이미 정해 두었다. 시그니처가 정해지기 전에는 조각 1을 열지 않는다.

**2026-08-06 — 집계 SELECT를 잠금 읽기로 못 박았다(PR #121 3차 리뷰 P1).** 잠금 순서를 고쳐 교착은 없앴지만 값은 여전히 어긋날 수 있었다. `REPEATABLE READ`의 읽기 스냅샷은 트랜잭션의 첫 평범한 SELECT에서 굳는데, A3는 잠금 **전에** A2의 검증 4가지를 평범한 SELECT로 돌린다. 그래서 잠금을 기다리던 트랜잭션은 잠금을 얻은 뒤에도 먼저 커밋된 후기를 못 보고 자기 INSERT만 반영한 값으로 덮어쓴다. `FOR UPDATE`로 current read를 강제한다. 잠금 순서 지적과 같은 뿌리지만 **순서만으로는 닫히지 않는 절반**이다.

**2026-08-06 — 답글도 상태 확인과 저장을 한 문장에 묶는다(PR #121 3차 리뷰 P2).** 2차 리뷰에서 C5에 `PUBLISHED` 검증을 넣었지만 확인과 INSERT가 여전히 별개 문장이라, 2.1이 이미 금지한 것과 같은 경합이 답글 경로에 남아 있었다. FK는 후기의 존재만 확인하므로 숨김이 먼저 커밋돼도 답글이 들어간다. 2.1의 규칙을 그대로 적용한다.
