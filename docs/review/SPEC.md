...
- **소유권과 기대 상태를 UPDATE 조건에 함께 넣는다.** `UPDATE reviews SET ... WHERE id = ? AND member_id = ? AND status = 'PUBLISHED'`로 쏘고 `affectedRows == 0`이면 그때 원인을 가려 던진다.
  - 2.1의 조건부 UPDATE는 **상태 전이**를 두고 쓴 규칙이라 평점·본문 수정은 그 밖에 있었다. 위 `PUBLISHED` 검증만으로는 관리자 숨김(C4)이 그사이에 커밋되는 것을 막지 못한다.
  - 그러면 **작성자가 숨겨진 후기의 내용을 덮어쓴다.** 관리자가 숨김을 해제하는 날 차단했던 것과 다른 글이 공개된다. 2.1이 "숨겨진 후기에 대해 작성자가 할 수 있는 일은 없다"고 정한 것과 정면으로 어긋난다.
  - 2.5의 소유권 판정에 조건 하나가 붙을 뿐이라 문장이 늘지 않는다.
...
**2026-08-06 — 다른 도메인 테이블 JOIN을 전면 금지한다(2.7 신설).** 3차에서 주문만 계약으로 돌렸더니 4차에서 관리자 검색(C2)의 `members`·`order_items`가 같은 자리로 다시 올라왔다. **표시용 조인은 허용하자는 안도 검토했다** — 커뮤니티가 `members`를 8곳에서 직접 JOIN하고 있어 선례가 그쪽이다. 택하지 않았다. 어디까지가 표시용인지를 매번 판단해야 하고, 그 판단이 한 번 느슨해지면 자격 검증 같은 업무 규칙이 같은 문으로 들어온다 — **A1·A2·A3가 실제로 그렇게 들어와 있었다.** 대신 필요한 값은 전용 QueryService가 DTO로 돌려주고, 목록은 ID 묶음 조회로 N+1을 피한다. 커뮤니티를 되돌리는 것은 이 문서 범위 밖이다.

**2026-08-06 — 제외·검색 조건은 계약이 페이징과 함께 처리한다(PR #121 4차 리뷰 P2).** 3차에서 A1을 계약으로 돌리면서 `NOT EXISTS`만 리뷰에 남겼는데, 계약이 먼저 `LIMIT 20`을 자르면 **최신 20건이 전부 작성 완료일 때 첫 페이지가 통째로 빈다.** 목록이 비는 것은 정상 상태라 오류로도 안 드러난다. C2 검색도 같은 모양이다. **리뷰가 뒤에서 거르지 않는다**를 2.7의 규칙으로 올리고, A1은 계약이 제외 ID 목록을 받는 시그니처를 제안한다.

**2026-08-06 — 평점 범위는 화면·서버·DB 세 겹으로 막고 데이터 보정 절차는 두지 않는다(PR #121 4차 리뷰 P2).** 별 5개만 렌더링해 0·6에 해당하는 입력을 애초에 만들지 않는 것이 첫 겹이다. **기본 선택은 두지 않는다** — `overall_rating`은 상품 평균으로 나가는 대표값이라 고객이 손대지 않은 별 1개가 집계에 섞인다. migration 사전 보정을 요구받았으나 택하지 않았다. `status`와 달리 평점 컬럼에는 `DEFAULT` 같은 체계적 발생원이 없고 `reviews`에 INSERT하는 코드가 저장소에 없으며, **6을 5로 깎는 "보정"은 고객 평가를 지어내는 것**이다. `CHECK`가 배포에서 멈추는 것이 올바른 동작이다.

**2026-08-06 — 후기 수정도 소유권과 상태를 한 UPDATE 조건에 넣는다(PR #121 4차 리뷰 P1).** 2.1의 조건부 UPDATE를 상태 전이만 두고 써서 A4(평점·본문 수정)가 규칙 밖에 있었다. 관리자 숨김이 그사이 커밋되면 **작성자가 숨겨진 후기의 내용을 덮어쓰고**, 해제하는 날 차단했던 것과 다른 글이 공개된다. 2.5의 소유권 판정에 조건 하나가 붙을 뿐이다.
