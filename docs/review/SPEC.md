...
**리뷰에는 ReadModel이 없다.** 15.9는 여러 도메인을 **집계·요약**하는 통계·대시보드에만 열린다. C1·C2 관리자 목록·검색은 조건이 다른 도메인에서 오더라도 결국 **후기 목록**이라 대상이 아니다 — 리뷰가 소유하고 조건은 계약으로 받는다.
...
- **관리자 화면이지만 ReadModel이 아니다.** 15.9는 집계·요약하는 통계·대시보드에만 열린다. 이건 조건이 다른 도메인에서 올 뿐 결국 후기 목록이다.
