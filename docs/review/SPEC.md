...
### 2.7 도메인 경계 — 다른 도메인 테이블을 JOIN하지 않는다

**리뷰 매퍼는 `reviews`·`review_replies`·`review_images` 외의 테이블을 JOIN하지 않는다. 예외를 두지 않는다.** 표시용이든 검색용이든 같다.
...
> **이 경로만 만들면 고객이 실제로 가는 화면에는 후기가 안 붙는다. 조각 3 착수 전에 시은님과 합의한다.**
>
> - `ProductController.detail`이 `GET /products/{id}`에서 `customer/product/detail.html`을 **서버에서 렌더링**하는데, 그 템플릿의 후기 영역은 `후기 목록은 준비 중입니다.`라는 **정적 문구**이고 후기를 가져오는 코드가 없다. 위 `GET /products/{id}/reviews`만 구현하면 **주소를 직접 넣은 사람에게만 후기가 보인다.**
> - 정할 것은 **기존 상세 요청이 리뷰 계약을 불러 모델에 담을지, 이 경로를 fragment/API로 부를지**다. 후자에 `CommunityApiController` 선례가 있고, B1은 페이징이 있어 상세 렌더링에 통째로 실으면 2페이지부터 갈 곳이 없다.
> - **`detail.html`과 `ProductController`는 시은님 담당이다.** 후기 영역 교체는 8절에 이미 올라와 있지만 어느 쪽 방식인지에 따라 `ProductController`까지 건드릴지가 갈린다. PR #74(주환)가 지금 같은 템플릿을 고치는 중이라 조각 3 착수 시점에 `dev` 기준을 다시 확인한다.
> - **검증에 "상품 상세 화면 자체가 후기를 렌더링하는지"를 넣는다.** `GET /products/{id}/reviews` 응답만 확인하면 이 구멍이 그대로 통과한다.
...
| **상품 상세 화면에 후기를 붙이는 방식** — 모델 주입이냐 fragment/API냐 | **조각 3 착수 전, 시은님과 합의** (B1) |
| **관리자 검색 계약**(`writer`·`product`) | **조각 5 착수 전, 수민·주환님과 합의** (C2·2.7) |
| D1 계약 시그니처·전용 매퍼 동의 | 조각 2 착수 전, 이슈 #33에서 확인 (방식 자체는 2026-08-06 확정) |
| **집계 SQL의 소유권** — D1은 `domain/product`의 `ProductReviewMapper`가 `reviews`를 읽게 하는데, 2.7이 금지하는 "다른 도메인 테이블 직접 조회"에 그대로 걸린다. 리뷰 소유 매퍼가 집계 DTO를 계산해 상품 공개 Service에 넘기는 안이 대안이다 | **조각 2 착수 전, #33에서 함께 정한다** (2.7·D1) |
