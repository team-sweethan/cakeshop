...
- `BLOCKED → DELETED` 금지: 숨긴 후기는 관리자 조치의 기록이다. 작성자가 지워서 없앨 수 없어야 한다. 결과적으로 **숨겨진 후기에 대해 작성자가 할 수 있는 일은 없다**(수정·삭제 모두 불가).
...
`SecurityConfig`는 이미 `/reviews/**` 인증 필요, `/products/**` 공개, `/admin/**` `hasRole("ADMIN")`이다. **경로를 아래처럼 두면 설정을 고칠 필요가 없다.**
...
- **모든 상태를 보여 준다.** 관리자는 숨긴 후기와 삭제된 후기도 봐야 조치 이력을 확인할 수 있다.
- 정렬 `created_at DESC, id DESC`.

**출력**: 작성자, 상품명, 평점(`overall_rating`), 내용 미리보기, 작성일, 상태, 관리 버튼

**목업에서 걷어낼 것**: `삭제` 버튼. **관리자 조치는 숨김뿐이다**(커뮤니티 선례). 관리자가 지울 수 있으면 조치 이력이 사라진다.
...
| 관리자 숨김 사유 기록 여부 | 필요해지면 새 migration |
| A6 이미지 첨부 | 2차 |
