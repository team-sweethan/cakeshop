...
| 화면 인벤토리 | 이 문서 3절 |
| 아직 안 정해진 것 | 이 문서 4절 |
| 조각 순서, 진행 상태, 위험, 결정 로그, 하네스 | `PLAN.md` |
...
| **E1** | `ReviewStatus` enum + `CHECK` | — | — | **완료** (#125) | 0 | `review-schema.md` |
| **E2** | 평점 범위 `CHECK` | — | — | **완료** (#125) | 0 | `review-schema.md` |
| **E3** | `Review`·`ReviewReply` 엔티티 | — | — | **완료** (#125) | 0 | `review-schema.md` |
...
| DB | 조각 0의 `CHECK (... BETWEEN 1 AND 5)`(E2(`specs/review-schema.md`)) | 위 둘을 지나온 것 — 최후의 그물 |
...
> **커뮤니티도 더 이상 `members`를 직접 JOIN하지 않는다** — 조각 10(PR #142·#143)이 8곳을 걷어내고 `MemberCommunityQueryService` 경유로 바꿨으며, `CommunityDomainBoundaryTests`(커뮤니티 H34)가 되돌아오는 것을 막는다. **리뷰가 이 선을 먼저 그었고 커뮤니티가 뒤따라온 것이다.**
>
> 선을 하나로 두는 이유는 그대로다. 표시용 조인까지 허용하면 어디까지가 표시용인지를 매번 판단해야 하고, 그 판단이 한 번 느슨해지는 순간 자격 검증 같은 업무 규칙도 같은 문으로 들어온다 — 실제로 A1·A2·A3가 그렇게 들어와 있었다.
>
> ReadModel 예외는 이 판단을 되살리지 않는다. 갈리는 지점이 **"표시용이냐"가 아니라 "읽기 전용이냐"**이기 때문이다. ReadModel은 쓰기 SQL을 아예 담지 못하고 그 결과를 변경 근거로 쓸 수도 없으므로, 자격 검증 같은 업무 규칙은 같은 문으로 들어올 수가 없다. 커뮤니티가 느슨해졌던 자리는 표시용 조인이 업무 판단으로 번진 것이었고, 그 경로가 닫혀 있다.
...
| D1 계약 시그니처·전용 매퍼 동의 | 조각 2 착수 전, 이슈 #33에서 확인 (방식은 `ADR-001`) |
