...
마이페이지 `작성할 후기` 링크가 가리키는 화면.

| | |
|---|---|
| 액터 | 로그인 고객 |
| 경로 | `GET /mypage/reviews/writable` |
| 화면 | `customer/review/writable.html` (조각 1에서 신설) |
| 입력 | `page`(선택, 기본 1) |
...
>
> - `docs/conventions.md` 12절은 다른 도메인의 테이블을 직접 JOIN·조회하지 않고 공개 Service나 QueryService를 거치도록 정한다. 위 문장은 review 쪽에서 `order_items`와 `orders`를 직접 JOIN하고, **A2·A3의 검증 1~3번도 같은 접근을 전제한다.** D1(`product-rating.md`)에서 상품 쪽에 적용한 기준을 주문 쪽에는 적용하지 않은 자리다.
...
- **판단 기준은 `content`가 비었는지이지 `totalElements`가 0인지가 아니다.** 항목이 있어도 마지막 페이지를 넘겨 요청하면 `content`만 비고 `totalElements`는 양수다. 건수로 판단하면 안내도 목록도 없고, `totalPages`가 작아 페이지 이동 UI까지 사라져 **아무것도 없는 화면에 갇힌다**(조각 1 Codex 지적).
...
- **진입점은 네 곳이다.** 처음에 셋으로 셌다가 화면 카탈로그를 놓쳐 그쪽만 400을 내는 채로 남았다(조각 1 Codex 지적).

| 진입점 | 조각 1에서 | 담당 |
|---|---|---|
| 마이페이지 `작성할 후기` | → `/mypage/reviews/writable` (A1) | 완료 |
| 상품 상세 `후기 작성` | → `/mypage/reviews/writable` (A1) | 완료 |
| 화면 카탈로그(`home/screens.html`) C16 | → `/mypage/reviews/writable` (A1) | 완료 |
| 주문 상세 `후기 작성` | 링크가 애초에 없다. 목록 반복 안에 `orderItemId`를 넣는 일이라 **주환님 몫** | 미착수 |

- **`/reviews/new`로 직접 보내는 진입점을 새로 만들지 않는다.** 파라미터를 빠뜨리면 400이라, A1을 거치게 하면 그 실수가 생길 자리가 없다.
...
**목업에서 걷어낸 것**: 같은 폼 안에 있던 `수정하기`·`삭제하기` 버튼(조각 1). 등록 화면과 수정 화면(A4(`review-edit-delete.md`))은 갈라진다.
...
> 조각 1에서는 B3가 없어 `/mypage/reviews/writable`(A1)로 보내고 **항목이 목록에서 빠진 것**으로 확인시켰다. 조각 3에서 B3가 나면서 옮겼다 — 없어진 것보다 생긴 것을 보여 주는 편이 확인이 확실하다.

## 2차

### A6. 이미지 첨부

작성 폼(A2)에서 이미지를 선택해 등록(A3)할 수 있다.

- 선택 입력이며 최대 3장, 장당 5MB 이하의 JPEG·PNG만 받는다.
- 확장자·MIME·파일 시그니처가 모두 같은 형식을 가리켜야 한다.
- 선택 순서대로 `review_images.sort_order` 0부터 저장한다.
- 파일을 먼저 전부 검증한 뒤 저장한다. 한 장이라도 잘못되면 파일과 이미지 행을 하나도 만들지 않는다.
- 후기 INSERT와 이미지 행 INSERT는 같은 트랜잭션이다. 뒤 작업이 실패해 rollback되면 이미 저장한 파일을 정리한다.
- 상품 후기 미리보기·전체 목록, 내 후기, 관리자 상세에서 본문 아래 순서 있는 첨부 목록으로 노출한다.
- 수정 화면(A4(`review-edit-delete.md`))에서 이미지 추가·삭제는 열지 않는다. 다만 **첨부한 이미지를 읽기 전용으로 보여 준다** — 안 보여 주면 작성자가 지워진 줄 안다(08-20). 숨김·soft delete 때도 이미지 행과 파일을 유지한다.
- 저장 위치는 공용 `FileStorageClient`와 `FileStorageDirectory.REVIEW`를 사용한다.
