...
> 조각: 2 (#33) — **PR #178 머지 완료**

### D1. 상품 평점 집계

> **이 기능은 이슈 #33 `feat(product): 리뷰 평점 및 후기 수 연동`(시은 담당)과 같은 일이었다.**
> 별도 이슈를 만들지 않고 #33에서 진행했고 착수 사실을 남겼다(허락이 아니라 중복 작업 방지).
> 상세는 `PLAN.md` 조각 2와 R9.
> `domain/product/`는 시은님 담당이지만 **만든 것이 전부 새 파일이고 기존 파일은 한 줄도 고치지 않아** 조각 1과 같은 순서를 따랐다 — 만들고 PR에서 확인받는다(`DOMAIN.md` 2.7, `docs/conventions.md` 12절).
...
**시은님께 확인받을 것으로 적어 두었던 목록** — 아래는 PR #178 시점의 것이고, 시그니처는 그대로 머지됐다. 나중에 이 계약을 바꿀 때 같은 항목을 다시 짚는다.
