...
| 9 | (2차) 알림 deep link | — | **진행 중** | 두 알림이 목록이 아니라 그 후기를 연다. 고객 쪽은 `GET /mypage/reviews/{id}`가 대상 후기를 첫 쪽 창에 넣는다(D3) |
...
`conventions.md` 12절과 `CLAUDE.md`는 **담당자가 쓴 코드를 고칠 때 합의가 선행한다**고 정한다.
