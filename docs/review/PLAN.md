...
| 8 | (2차) 이미지 첨부 | — | **구현 완료 (검증 대기)** | 작성 시 최대 3장, 5MB JPG·PNG, 고객·관리자 화면 노출, `review_images` |
...
**알림 도메인 파일을 고친다.** 두 링크 모두 `NotificationResponse.getTargetUrl()`이 파생시키므로
민정님 파일이 갈린다. PR #322가 같은 자리를 같은 방식으로 이미 고쳤고, 확인은 PR 리뷰어 지정으로
받는다(`docs/pull-request.md` 1절).
