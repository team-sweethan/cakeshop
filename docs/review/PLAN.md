...
|---|---|---|---|---|
| H1 | `docs/review/**/*.md`와 `domain/review/**/*.java` 주석이 가리키는 문서 경로가 실존함 | `ReviewDocTests` — `docs/review/...` 전체 경로와 `specs/`·`decisions/`·`history/` 상대 경로 양쪽을 본다 | **경로가 맞다는 것뿐이다.** 그 문서에 실제로 그 내용이 있는지는 보지 않는다 | **적용** (문서 재배치) |
