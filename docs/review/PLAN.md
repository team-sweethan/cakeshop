...
문서 PR #121에 Codex 리뷰가 5라운드 돌았다(10 → 5 → 7 → 5 → 3건). **전부 실재하는 지적이었다.** 상태값 정본이 열려 있다는 것과 local preview가 `/reviews/**`를 공개한다는 것은 코드를 열어 확인했다.

| 라운드 | 정한 것 | 정본 |
|---|---|---|
| 1차 | 상태 전이의 **판단과 쓰기를 한 문장에** 묶는다 — `WHERE ... AND status = ?` 후 `affectedRows` 판정 | SPEC 2.1 |
| 1차 | 집계는 **다섯 호출 지점 모두** 후기 쓰기와 같은 트랜잭션. 평균은 `COALESCE(AVG(...), 0)` | SPEC D1 |
| 1차 | 후기 저장 순서를 **잠금 → INSERT → 집계**로. 뒤집으면 FK 공유 잠금이 배타로 승격되어 교착 | SPEC A3·D1 |
| 1차 | 조각 1에서 local preview 목록에서 `/reviews/**`를 뺀다. `rds`는 닫혀 있어 **로컬에서만 뚫려 있다** | SPEC 2.3 |
| 1차 | 답글은 `PUBLISHED` 후기에만. 관리자 목록 `조치 이력` 근거를 걷어냈다 — `blocked_at`이 없다 | SPEC C5·C1 |
| 2차 | 판매 중지 상품의 후기는 공개하지 않는다. `NOT_ON_SALE` 400을 404로 바꾼다 | SPEC B1 |
| 3차 | 주문 데이터도 공개 계약으로 받는다. D1의 기준을 주문 쪽에는 적용하지 않은 자리였다 | SPEC A1 |
| 3차 | 집계 SELECT를 **잠금 읽기**로. 순서만으로는 `REPEATABLE READ` 스냅샷이 닫히지 않는다 | SPEC D1 |
| 3차 | 답글도 상태 확인과 저장을 한 문장에 묶는다 | SPEC C5 |
| 4차 | **다른 도메인 테이블 JOIN 전면 금지**(2.7 신설). 표시용 예외를 두지 않는다 | SPEC 2.7 |
| 4차 | 제외·검색 조건은 계약이 페이징과 함께 처리한다. 리뷰가 뒤에서 거르면 건수가 틀어진다 | SPEC A1·2.7 |
| 4차 | 후기 수정도 소유권과 상태를 한 UPDATE 조건에 넣는다 | SPEC A4 |
| 4차 | 평점은 화면·서버·DB 세 겹으로 막고 **기존 데이터 보정 절차는 두지 않는다** | SPEC 2.2 |

**남은 5차 3건은 열려 있다** — 집계 매퍼의 소유권(2.7과 D1이 반대 방향), B3·C1·C3용 주문 스냅샷 계약, 수정 시 무조건 재집계. 앞의 둘은 조각별 선행 합의에 이미 걸려 있다.

**2026-08-06 — 문서 중복을 걷어냈다.** SPEC 결정 로그를 이 문서로 합치고, FLOW는 흐름만, 도메인 `CLAUDE.md`는 파일 범위·담당자·점검 목록만 남겼다. 같은 규칙이 세 곳에 적혀 있어 한 곳을 고치면 세 곳을 고쳐야 했고, 어긋난 사본 자체가 리뷰 지적이 되고 있었다(2·3차의 FLOW 지적 2건). **규칙과 근거는 SPEC 본문에 한 번만 둔다.**
