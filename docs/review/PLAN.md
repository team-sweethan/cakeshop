...
| H2 | `DOMAIN.md` 1절 기능 표의 모든 ID가 `spec` 열이 가리키는 파일에 `### <ID>.`로 **정확히 한 번** 나옴. **1절 표 안에 같은 ID 행이 둘 이상 없는 것도 함께** | `ReviewDocTests` — 라우팅을 `Map`으로 읽으므로 중복 행은 뒤가 앞을 조용히 덮어쓴다. 그러면 두 행이 서로 다른 spec을 가리켜도 H2·H3가 전부 통과한다(PR #154 Codex 리뷰) | 절의 **내용**은 보지 않는다. 빈 절이어도 통과한다 | **적용** (문서 재배치) |
