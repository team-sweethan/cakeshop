...
| 08-10 | **`NEW_REVIEW`를 활성 관리자 전원에게 각각 보낸다.** `DOMAIN.md` 4절의 미정 항목이었는데, **담당자가 세운 전제가 이미 있었다** — 채팅 스키마(PR #149)가 `관리자 계정 하나가 전체 채팅방을 관리하므로` 라며 `chat_rooms.admin_id`를 걷었고 seed의 관리자도 하나다. 그 전제 아래서 "전원"은 곧 그 하나라 결과가 같으면서, 관리자가 둘이 되어도 깨지지 않는다. 관리자 조회는 `role`·`status`라는 회원 도메인의 사실이라 member가 맡는다 | `specs/review-notification.md` D2 |
