...
**자동 검증 체계의 정본은 `HARNESS.md`다.** 지금 적용된 것은 H1~H6이고, 조각 3이 H7~H9를 세운다.

여기에는 표를 다시 적지 않는다 — **이번 조각이 추가·변경하는 번호만** 각 조각 절에 적는다. `PLAN.md`가 부르는 번호가 `HARNESS.md`에 실존하는지는 H6이 본다. 그 표가 다시 이 문서로 새어 들어오는 것은 H4가 막는다.
