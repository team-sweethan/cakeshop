...
| 1 | 작성 | #109 | **진행 중** | 자격 검증(`PICKED_UP` + 본인 주문), `reviews` INSERT, 주문상품당 1건, **작성할 후기 목록 화면 신규**(A1) |
...
**이 조각이 세우는 하네스: H10 ~ H13** (`HARNESS.md`).

- **H10** — 작성 자격 검증 넷의 순서와 오류 코드, `product_id`를 계약이 준 값에서 파생시키는 것
- **H11** — 평점 1~5와 미선택, 본문 10~2000자를 **서버가** 거부하는 것
- **H12** — 작성 화면 둘의 실제 렌더링. **빈 목록 분기를 포함한다** — 평소 화면에 없는 것은 표현식이 깨져도 아무도 모른다
- **H13** — `orderItemId`가 없거나 숫자가 아니면 **400**. 명세가 약속한 상태 코드가 실제로 나가는지 본다
...
| 08-09 | 주문 상세의 `후기 작성` 진입점을 조각 1에서 빼고 주환님께 넘겼다. 링크가 애초에 없었고, 목록 반복 안에 `orderItemId`를 넣는 일이라 링크 한 줄 수정을 넘는다 | `specs/review-write.md` A2 |
