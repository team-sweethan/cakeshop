...
> **여기는 조각 1과 달리 착수 전에 #33에 계약 시그니처를 올려 확인받는다.** 새 파일이라 `AGENTS.md`의 사전 협의 대상은 아니지만, 이유가 둘 더 있다.
>
> - **쓰기 계약이다.** `products` 컬럼을 UPDATE 하고 잠금 순서가 걸린다. `conventions.md` 15.7이 잠금 순서와 중복 실행 방지를 **구현 전에 합의**하라고 정한다 — 어긋나도 단일 요청에서는 결과가 같아 테스트가 통과한다.
