...
> 흐름의 정본은 `docs/review/FLOW.md`다. 규칙이 확정되면 `docs/review/DOMAIN.md`를 세우고 그쪽이 정본이 된다.
...
| # | 조각 | 상태 | 내용 |
|---|---|---|---|
| 0 | 준비 | 대기 | `ReviewStatus` enum, `CHECK` 제약 migration, 평점 범위 제약, `status` 기본값 정리 |
| 1 | 작성 | 대기 | 자격 검증(`PICKED_UP` + 본인 주문), `reviews` INSERT, 주문상품당 1건 |
| 2 | 평점 집계 | 대기 | `products.average_rating`·`review_count` 재계산. **1 직후에 붙인다** |
| 3 | 조회·노출 | 대기 | 상품 상세 후기 목록, 페이징, 내 후기 조회 |
| 4 | 수정·삭제 | 대기 | soft delete, 집계 재호출 |
| 5 | 관리자 숨김 | 대기 | 관리자 목록, `BLOCKED` 전이, 집계 재호출 |
| 6 | 관리자 답글 | 대기 | `review_replies`, 후기당 1건 |
| 7 | 알림 연동 | 대기 | `NEW_REVIEW`, `CUSTOMER_REVIEW`. **PR #107 머지 이후** |
...
- `UPDATE products SET average_rating = (SELECT ...), review_count = (SELECT COUNT(*) ...), updated_at = updated_at WHERE id = ?`
- **재계산 방식**을 쓴다 (`docs/community/DOMAIN.md` 6.5 선례). 후기는 좋아요보다 훨씬 적게 쌓이므로 증분의 이점이 없고, 앞으로 갱신 경로가 늘 때마다(수정·삭제·숨김) "여기서도 조정해야 하나"를 판단하지 않아도 된다.
- **`updated_at = updated_at` 보존이 이 문장의 일부다.** 커뮤니티에서 같은 자리를 두 번 빠뜨려 화면에 `(수정됨)`이 붙는 버그가 실제로 났다.
- 잠금 순서를 따진다 — `reviews` INSERT가 FK로 `products` 행에 공유 잠금을 걸고 재계산이 배타 잠금을 요구한다. 커뮤니티 좋아요와 같은 모양이므로 같은 해법(`SELECT ... FOR UPDATE` 선행)을 쓴다.

**검증**: 작성 후 집계 일치, **동시 요청 후 `review_count == reviews 실제 개수`**, 집계 갱신이 `products.updated_at`을 건드리지 않는지.
