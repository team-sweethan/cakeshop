...
| 1 | 작성 | #109 | **구현 완료** (PR #177) | 자격 검증(`PICKED_UP` + 본인 주문), `reviews` INSERT, 주문상품당 1건, **작성할 후기 목록 화면 신규**(A1). **조각 2와 함께 머지한다** |
| 2 | 평점 집계 | **#33** | **구현 완료** (PR #178) | 집계 연동. **조각 1과 한 배포 단위.** 시은 담당 이슈와 같은 일이라 착수 시 #33에 알린다 |
...
> **순서는 조각 1과 같다 — 만들고 PR에서 확인받는다.** 만드는 것이 전부 새 파일이고 시은님 파일을 한 줄도 고치지 않기 때문이다(`DOMAIN.md` 2.7).
>
> - **쓰기 계약이라고 달라지지 않는다.** 12절이 가르는 기준은 읽기·쓰기가 아니라 **깨질 남의 호출부가 있느냐**다. 다만 `conventions.md` 6절대로 **잠금 순서·rollback 범위·중복 실행 방식은 구현 전에 정한다** — 어긋나도 단일 요청에서는 결과가 같아 테스트가 통과한다. 그 답은 `specs/product-rating.md`에 이미 있고, 재고 경로와의 정합도 `ProductStockService`를 읽어 확인한 것이라 대기가 필요 없다.
> - **#33에는 착수 사실을 알린다.** 같은 일이라 모르고 만들면 두 사람이 같은 것을 두 번 만든다. **허락이 아니라 중복 작업 방지이므로 답을 기다리지 않는다.**
...
**새로 만들 계약 2건.** 둘 다 남의 도메인에 **새 파일로만** 추가하므로 착수를 막지 않는다 — 만들고 PR 리뷰어로 확인받는다(`DOMAIN.md` 2.7).

- **수민님** — 작성자 표시명 계약(ID 묶음 조회). `members`를 JOIN하지 않는다
- **주환님** — B3·C1·C3용 주문 스냅샷 계약. **`OrderReviewQueryService`는 *미작성* 항목만 돌려주므로 이미 쓴 후기의 상품명·주문번호를 받을 자리가 없다**(`DOMAIN.md` 4)
...
| R9 | **조각 2는 상품 도메인(시은 담당)에 파일을 만든다.** 전부 새 파일이라 사전 협의 대상은 아니지만(12절), 시그니처가 리뷰에서 뒤집히면 리뷰 쪽 호출부가 따라 움직인다. **`ProductReviewCommandService`의 두 번째 메서드는 이름·인자가 아직 없다** | 착수 시 시그니처를 확정하고 **시은님을 PR 리뷰어로 지정**, #33에는 착수 사실을 알린다 (`specs/product-rating.md` 끝 절, `decisions/ADR-001-rating-aggregation-ownership.md`) (2026-08-10 개정) |
