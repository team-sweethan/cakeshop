...
| 4 | 수정·삭제 | #111 | **구현 완료** | soft delete, 집계 재호출, **수정 폼 화면 신규**(A4), B3에 수정·삭제 버튼 |
...
**동시 실행 검증에 빠진 자리가 하나 있다.** `DOMAIN.md` 2.1이 든 예는 **작성자 삭제(A5)와 관리자 숨김(C4)**이 겹치는 경우인데 C4가 아직 없어 그대로 재현할 수 없다. 지금 있는 것은 매퍼 계층의 "`BLOCKED` 행은 조건부 쓰기가 못 건드린다"와 서비스 계층의 원인 판별, 그리고 수정↔삭제 동시 실행이다. **C4가 생기는 조각 5에서 그 조합을 동시 요청 테스트로 고정한다.**
