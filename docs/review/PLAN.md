...
|---|---|---|---|---|
| H1 | `docs/review/**/*.md`와 `domain/review/**/*.java` 주석이 가리키는 문서 경로가 실존함 | `ReviewDocTests` — 문서에서는 `docs/`·`../`·`specs/`·`decisions/`·`history/`로 시작하는 것만, 자바 주석에서는 `docs/`로 시작하는 것만 본다 | **경로가 맞다는 것뿐이다** — 그 문서에 그 내용이 있는지는 보지 않는다. **맨 이름은 아예 보지 않는다**(`PLAN.md` R2 같은 산문 표기를 위반으로 만들지 않으려고 좁혔다). `domain/review/CLAUDE.md` 같은 줄임 표기도 검사 밖이다. **없어진 문서를 이름으로 불러도 되는 자리는 아래 migration을 함께 적은 줄뿐이다** — 이름이 아니라 줄로 좁혔다(PR #154 Codex 리뷰) | **적용** (문서 재배치) |
| H2 | `DOMAIN.md` 1절 기능 표의 모든 ID가 `### <ID>.` 절로 **`specs/` 전체에 정확히 한 번** 나오고, 그 하나가 `spec` 열이 가리키는 파일임. **1절 표 안에 같은 ID 행이 둘 이상 없는 것도 함께** | `ReviewDocTests` — 라우팅을 `Map`으로 읽으므로 중복 행은 뒤가 앞을 조용히 덮어쓴다. 그러면 두 행이 서로 다른 spec을 가리켜도 H2·H3가 전부 통과한다(PR #154 Codex 리뷰). 세는 범위가 라우팅된 파일 안이면 **같은 절을 다른 spec에 복제한 것**을 놓친다(2차) | 절의 **내용**은 보지 않는다. 빈 절이어도 통과한다 | **적용** (문서 재배치) |
| H3 | 인벤토리 3자 일치 — `DOMAIN.md` 1절 ID 집합 = `specs/`의 `### <ID>.` 집합, 그리고 1절 조각 번호 집합 = `PLAN.md` 조각 표 집합 | `ReviewDocTests` — 조각 쪽도 **양방향**이다. 한 방향이면 `PLAN.md`에 조각 행만 늘리고 1절·specs를 안 고쳐도 통과한다(PR #154 Codex 2차). 기능 없는 조각은 예외로 두지 않는다 — 조각 0도 스키마 준비였는데 1절에 E1~E3를 받았다 | `현재` 열이 실제 코드와 맞는지는 **보지 않는다.** 그쪽은 조각 3의 2단계 검사가 맡는다 | **적용** (문서 재배치) |
...
그리고 그 사실을 문서에 적으려면 **없어진 이름을 불러야 한다.** 그래서 H1은 위 migration 이름을 함께 적은 줄만 건너뛴다. 처음에는 없어진 두 문서의 **이름 자체**를 어디서 부르든 통과시켰는데, 그러면 다른 문서에 낡은 참조가 새로 들어와도 H1이 그냥 넘긴다 — 막으려던 죽은 참조가 그 이름으로만 뚫린다(PR #154 Codex 리뷰). **예외는 이름이 아니라 줄에 건다.** 이 문단 밖에서 그 문서를 이야기할 때는 전체 경로가 아니라 맨 이름(`SPEC.md`)으로 적는다 — 맨 이름은 H1이 경로로 보지 않는다.
