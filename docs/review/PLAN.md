...
| 08-06 | 주문 상세에서는 **이미 후기를 쓴 항목도 링크를 가리지 않고** 폼에서 409로 막는다. 가리려면 주문 상세가 `reviews`를 읽어야 해서 경계를 넘는다 | SPEC A2 |
| 08-06 | 요청 파라미터 누락·타입 불일치 400을 **리뷰 오류 코드가 아니라 `GlobalExceptionHandler`**에 둔다. 리뷰만의 문제가 아니고, 도메인마다 같은 코드를 만들면 응답이 갈린다 | SPEC A2 |
