...
- **H1~H5가 무는 것을 스크립트가 아니라 테스트로 박는다** — 검사 뿌리를 픽스처 디렉터리로 받게 바꾸고, PR #154에서 나온 변형 10건을 회귀 테스트로 남긴다. 근거는 하네스 절 끝
...
이 문단 밖에서 그 문서를 이야기할 때는 전체 경로가 아니라 맨 이름(`SPEC.md`)으로 적는다 — 맨 이름은 H1이 경로로 보지 않는다.
...
**반복을 누적으로 바꾸는 것은 조각 3에 있다.** 지금은 라운드마다 사람이 변형을 심어 빨간불을 확인하고 되돌린다 — 그러면 **생각해 낸 변형만** 검증된다. 위 10건을 영구 회귀 테스트로 박으면 다음 사람이 검사를 리팩터링해도 같은 구멍이 다시 물린다. 검사들이 실제 경로 상수를 직접 읽고 있어 뿌리를 픽스처로 받도록 바꿔야 하고, 그 작업은 조각 3의 2단계 하네스와 함께 한다.
