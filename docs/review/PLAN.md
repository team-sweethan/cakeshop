...
| 1 | 작성 | #109 | **구현 완료** (PR 대기) | 자격 검증(`PICKED_UP` + 본인 주문), `reviews` INSERT, 주문상품당 1건, **작성할 후기 목록 화면 신규**(A1). **조각 2와 함께 머지한다** |
| 2 | 평점 집계 | **#33** | **구현 완료** (PR 대기) | 집계 연동. **조각 1과 한 배포 단위.** 시은 담당 이슈와 같은 일이라 착수 시 #33에 알린다 |
