...
> 이 문서는 **작업 순서, 진행 상태, 위험, 결정 로그, 하네스**만 다룬다.
...
**`history/`로 옮기는 기준: 머지되었고 지금도 구속하지 않는 것만.** 조각의 착수 항목과 검증 목록이 그렇다. 하네스 표·위험·결정 로그는 과거 기록처럼 보여도 **현재 구속**이므로 대상이 아니다.
...
**이 조각에 하네스 2단계를 함께 넣는다. 별도 작업으로 미루지 않는다** — 이 저장소에는 미룬 하네스가 안 온 기록이 있다(커뮤니티 H1b를 10c에서 미뤘다가 10d에서야 했다).

- `DOMAIN.md` 2.5 오류 코드 표 ↔ `ReviewErrorCode` — 코드·메시지·HTTP 상태. **코드에 있는데 표에 없으면 실패**
- `DOMAIN.md` 2.3 경로 표 ↔ 컨트롤러 매핑. 매핑이 있는데 1절 기능 표 `현재` 열이 `없음`이면 실패
- `DOMAIN.md` 3절 화면 표 ↔ `templates/{customer,admin}/review/**` 실존
- 화면 명세를 `screens/`로 나누고 커뮤니티 `CommunityScreenDocTests`에 해당하는 문구 대조 하네스를 세운다
...
**하네스 2단계의 나머지가 여기 붙는다** — 조각 5가 추가하는 오류 코드·경로·화면이 위 세 검사에 그대로 걸린다. 조각 3에서 세운 검사가 조각 5의 신규 항목을 실제로 무는지 확인한다.
...
조각을 진행하며 여기에 쌓는다. **각 행은 무엇을 보증하지 *않는지*까지 적는다** — 적지 않으면 다음 사람이 검사 범위를 넓게 믿는다. 커뮤니티에서 그런 문장이 여섯 건 틀렸다(PR #146).

| # | 무엇을 고정하나 | 형태 | 보증하지 않는 것 | 상태 |
|---|---|---|---|---|
| H1 | `docs/review/**/*.md`와 `domain/review/**/*.java` 주석이 가리키는 문서 경로가 실존함 | `ReviewDocTests` — 문서에서는 `docs/`·`../`·`specs/`·`decisions/`·`history/`로 시작하는 것만, 자바 주석에서는 `docs/`로 시작하는 것만 본다 | **경로가 맞다는 것뿐이다** — 그 문서에 그 내용이 있는지는 보지 않는다. **맨 이름은 아예 보지 않는다**(`PLAN.md` R2 같은 산문 표기를 위반으로 만들지 않으려고 좁혔다). `domain/review/CLAUDE.md` 같은 줄임 표기도 검사 밖이다 | **적용** (문서 재배치) |
| H2 | `DOMAIN.md` 1절 기능 표의 모든 ID가 `spec` 열이 가리키는 파일에 `### <ID>.`로 **정확히 한 번** 나옴 | `ReviewDocTests` | 절의 **내용**은 보지 않는다. 빈 절이어도 통과한다 | **적용** (문서 재배치) |
| H3 | 인벤토리 3자 일치 — `DOMAIN.md` 1절 ID 집합 = `specs/`의 `### <ID>.` 집합, 그리고 1절의 조각 번호가 전부 `PLAN.md` 조각 표에 있음 | `ReviewDocTests` | `현재` 열이 실제 코드와 맞는지는 **보지 않는다.** 그쪽은 조각 3의 2단계 검사가 맡는다 | **적용** (문서 재배치) |
| H4 | 역할 침범 — `specs/`에 결정 로그 표가 없고, `PLAN.md`에 `### <ID>.` 기능 절이 없음 | `ReviewDocTests` | 문서가 **역할대로 쓰였는지**는 형태로만 본다. 규칙을 두 곳에 풀어 적은 것은 못 잡는다 | **적용** (문서 재배치) |
| H5 | **이 표의 각 행이 가리키는 테스트 클래스·메서드가 실존함** | `ReviewDocTests` | 그 테스트가 **적힌 대로 검사하는지**는 못 본다. 클래스와 메서드가 있는지만 본다 | **적용** (문서 재배치) |

**H5를 처음부터 두는 이유**: 커뮤니티는 하네스를 36개 쌓고 나서야 표와 실제 테스트를 대조할 수단이 없다는 것을 발견했다(`docs/community/PLAN.md` 조각 11). 표가 자란 뒤에 붙이면 그때까지 쌓인 어긋남을 한꺼번에 갚아야 한다.

**H1이 일부러 보지 않는 곳이 하나 있다 — 머지된 Flyway migration.** `V20260806_075114__add_review_status_and_rating_constraints.sql`의 주석 세 곳이 지금은 없어진 `docs/review/SPEC.md`를 가리킨다. **고칠 수 없다** — Flyway가 파일 체크섬을 검증하므로 주석 한 글자만 바꿔도 이미 반영된 DB에서 배포가 실패한다. 검사 대상에 넣으면 영원히 빨간불이고, 그러면 사람이 검사를 끈다. **낡은 참조를 알면서 남겨 두는 유일한 자리이고, 여기 적어 두는 것이 그 대가다.**

**형태 검사는 낡은 참조를 잡지 틀린 설명을 못 잡는다.** "D1이 4곳에서 불린다"(실제 5곳) 같은 것은 H1~H5를 전부 통과한다. 그쪽은 리뷰가 맡는다.
...
| 08-07 | 문서를 `DOMAIN`/`specs`/`PLAN`/`decisions`/`history`로 나누고 `FLOW.md`·`SPEC.md`를 없앴다. 동시에 `ReviewDocTests`(H1~H5)를 세웠다 | 이 문서 하네스 절 |
