# cakeshop 코드 컨벤션 v2 (수정안)

- **상태**: 팀 합의 전 제안안 (v2 초안의 검토 반영본)
- **범위**: Java · Spring MVC · MyBatis · 패키지 · DB 스키마 · 공통 코드 기준
- **제외**: 테스트 작성 규칙, CI, PR, Flyway·seed 규칙 — [22. 별도 문서에서 결정할 항목](#22-별도-문서에서-결정할-항목) 참고
- **관련 문서**: 화면 규격 [frontend-template-format.md](frontend-template-format.md), 상태 설계 문서(분리 예정 — [14절](#14-상태값-규칙) 참고)

---

## 1. 문서 목적과 적용 원칙

이 문서는 도메인별 구현 방식이 달라지는 것을 막고 코드 리뷰의 공통 기준을 정하기 위한 제안안이다. 팀 합의 후 기존 conventions.md를 대체한다. (단, 상태값 인벤토리 절은 상태 설계 문서로 분리될 때까지 유효하다.)
...
> **이관 안내**: 도메인별 상태값 인벤토리, 담당자별 미확정 ☐ 항목(product_options·payment_cancellations·coupons·comments·reviews·chat_rooms·NotificationType), 함정 분류표는 **별도 상태 설계 문서**로 분리한다. 분리가 완료되기 전까지는 기존 conventions.md의 "상태값(status) 공통 규칙" 절이 그대로 유효하며, 담당자 ☐ 항목의 확정·갱신도 거기서 계속한다.
...
1. 팀이 이 제안을 검토·수정 → 합의 내용을 conventions.md 정본으로 반영한다.
