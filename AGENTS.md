...
도메인끼리는 테이블·Mapper·Entity를 직접 참조하지 않고 공개 Service 계약으로만 오간다. **연동 계약의 코드와 SQL은 쓰는 쪽이 아니라 데이터를 소유한 도메인에 둔다.** 여러 도메인을 **집계·요약**하는 통계·대시보드 조회만 ReadModel로 예외를 연다 — 관리자 화면이라는 것은 근거가 아니다. 규칙과 명명의 정본은 `docs/conventions.md` 15절이다.

## 도메인 담당과 협업 경계

담당자는 해당 도메인의 최종 오너이자 우선 리뷰 대상이다. 담당 외 도메인도 작업할 수 있고, **무엇을 먼저 협의해야 하는지는 남의 코드가 깨지는지로 가른다.**

| | 언제 확인받나 |
|---|---|
| 남의 도메인에 **연동 계약을 새 파일로 추가** | **사전 협의 없이 만들고** 그 담당자를 리뷰어로 지정해 PR에서 확인받는다 |
| 기존 **공개 Service 인터페이스 변경**·상태 전이 | **먼저 협의한다** |
| 여러 도메인을 JOIN하는 **ReadModel** | 최초 작성과 주요 변경 때 읽는 테이블의 담당자 확인을 받는다 |

새 파일 추가는 아무것도 깨뜨리지 않는다. 조건은 **담당자의 기존 파일을 수정하지 않는 것** 하나이고, 이름은 `<소유 도메인><참조 도메인>` 규칙을 따른다(2026-08-07 팀 합의, `docs/conventions.md` 15.2·15.8). 사전 합의를 필수로 걸면 6명이 서로의 답을 기다리며 멈추는데, 그것이 막는 위험은 "시그니처를 다시 고치는 것"뿐이라 값이 맞지 않는다.

반대로 이미 있는 공개 인터페이스와 상태 전이는 **남의 호출부가 함께 깨지므로** 먼저 협의한다. ReadModel은 테이블 구조가 바뀌면 조용히 어긋난다.
