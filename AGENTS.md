...
이 파일은 Codex와 Claude Code가 공유하는 **프로젝트 진입점**이다. 모든 세부 규칙을 복제하지 않고,
항상 필요한 불변 규칙과 작업별 정본의 위치만 제공한다. 규칙 하나는 한 정본에만 둔다.
...
- 테스트 계층·통합 범위·동시성·fixture·하네스를 **결정해야 할 때** `docs/testing.md`의 관련 절
- DB·Flyway·seed 구조를 바꿀 때 `docs/conventions.md` 7~8절과 관련 `docs/schema/*.md`
...
- 운영 코드는 `src/main/java/com/cakeshop`, MyBatis XML은 `src/main/resources/mapper/<domain>`, 화면은
  `src/main/resources/templates/{admin,customer}`, 테스트는 `src/test/java`에 둔다.
...
- 공유된 Flyway migration은 수정하지 않는다. 새 migration은
  `.\gradlew.bat newMigration -Pdesc=<snake_case>`로 생성한다.

## 도메인 경계와 담당

도메인은 다른 도메인의 테이블·Mapper·Entity를 직접 사용하지 않고, 데이터 소유 도메인의 공개 Service와
최소 DTO를 통해 연동한다. 여러 도메인을 집계·요약하는 통계·대시보드만 ReadModel 예외를 검토할 수 있다.
관리자 화면이라는 이유만으로 예외가 되지는 않는다. 상세 규칙과 이름은 `docs/conventions.md` 12절이 정본이다.
...
연동 계약의 위치와 이름, 사전 협의와 PR 확인의 구분, ReadModel 확인 기준은 `docs/conventions.md` 12절을
그대로 따른다. PR에서의 합의 기록, 리뷰어 지정과 작성자·담당자 헤더는 `docs/pull-request.md` 1절을 따른다.
이 파일에서 그 기준을 다시 요약하거나 넓혀 해석하지 않는다.
...
테스트 이름과 계층별 책임은 `docs/testing.md`를 따른다. DB 동작은 H2로 대체하지 않고 MariaDB
Testcontainers 기반으로 검증한다. 정상 흐름뿐 아니라 입력 검증, 권한, rollback, 중복 실행, 금지된 상태
전이를 위험에 비례해 확인한다.

## 보안과 데이터 무결성

- 요청으로 전달된 회원 ID를 소유권의 근거로 신뢰하지 않는다. Service에서 인증 사용자와 자원 소유권을
  검증한다.
- 관리자 기능은 화면 숨김이 아니라 Spring Security의 `ADMIN` 권한으로 강제한다.
- 비밀번호, 인증 정보, 개인정보, SQL과 내부 경로를 응답이나 로그에 노출하지 않는다.
- MyBatis 사용자 입력은 `#{}`로 바인딩한다. 동적 정렬은 허용값을 `<choose>`로 매핑하고 `${}`를 사용하지
  않는다.
...
해당 라우터를 통해 필요한 규칙만 추가로 읽으며, 이 파일에서 리뷰 기준을 다시 요약하지 않는다.
