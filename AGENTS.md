...
담당자는 해당 도메인의 최종 오너이자 우선 리뷰 대상이다. 담당 외 도메인도 작업할 수 있지만, 공개 Service 인터페이스나 상태 전이처럼 다른 담당자에게 영향을 주는 변경은 먼저 협의한다. **여러 도메인을 JOIN하는 ReadModel도 최초 작성과 주요 변경 때 읽는 테이블의 담당자 확인을 받는다** — 테이블 구조가 바뀌면 조용히 어긋난다.
