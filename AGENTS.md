...
새 파일 추가는 아무것도 깨뜨리지 않는다. 조건은 **담당자의 기존 파일을 수정하지 않는 것** 하나이고, 이름은 `<소유 도메인><참조 도메인>` 규칙을 따른다. 반대로 이미 있는 공개 인터페이스와 상태 전이는 **남의 호출부가 함께 깨지고**, ReadModel은 테이블 구조가 바뀌면 조용히 어긋난다.

**이 표의 근거와 예외, 쓰기 계약의 취급은 `docs/conventions.md` 15.2·15.7·15.8이 정본이다**(2026-08-08 팀 합의).
