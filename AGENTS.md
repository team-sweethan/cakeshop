...
도메인끼리는 테이블·Mapper·Entity를 직접 참조하지 않고 공개 Service 계약으로만 오간다. **연동 계약의 코드와 SQL은 쓰는 쪽이 아니라 데이터를 소유한 도메인에 둔다.** 여러 도메인을 조합해야 하는 통계·검색 조회만 ReadModel로 예외를 연다. 규칙과 명명의 정본은 `docs/conventions.md` 15절이다.
