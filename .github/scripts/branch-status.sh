...
# 마크다운 표의 일반 텍스트 셀. 사람 이름과 커밋 요약이 여기로 온다. '|' 가 들어오면
# 열 구분자로 읽혀 행이 깨진다.
#
# 백슬래시를 먼저 겹쳐 두지 않으면 원래 '\|' 이던 자리가 '\\|' 이 되고, GFM 은 앞
# 백슬래시가 뒤 백슬래시를 이스케이프한 것으로 읽어 파이프가 다시 구분자가 된다.
#
# '<' '>' '&' 도 막는다. GFM 은 마크다운 안의 raw HTML 을 허용하므로 커밋 제목의
# '<a href="x">' 가 태그로 해석되어 제목 일부가 화면에서 사라진다. HTML 출력은 같은
# 값을 html() 로 이스케이프해 글자 그대로 보여 주므로, 막지 않으면 두 문서가 같은
# 커밋을 다르게 보여 준다. 백슬래시 이스케이프는 GFM 에서 ASCII 구두점에 통하므로
# 엔티티 대신 이 방법을 쓴다. 앞서 겹친 백슬래시가 다시 겹치지 않도록 마지막에 둔다.
cell() {
    oneline "$1" | sed -e 's/\\/\\\\/g' -e 's/|/\\|/g' -e 's/[<>&]/\\&/g'
}

# 마크다운 표에서 백틱으로 감싸는 셀. 브랜치명이 여기로 온다.
#
# 코드 스팬 안에서는 '<' '&' 가 글자 그대로 나오고 백슬래시 이스케이프도 통하지 않아,
# cell() 을 쓰면 백슬래시가 화면에 그대로 보인다. 파이프만 막으면 된다. 표는 인라인
# 해석보다 먼저 '\|' 를 처리하므로 코드 스팬 안에서도 이 이스케이프가 통한다.
# git 은 refname 에 백슬래시를 허용하지 않으므로 백슬래시 처리는 필요 없다.
ref_cell() {
    oneline "$1" | sed -e 's/|/\\|/g'
}
...
md_divergence_rows() {
    ...
    while IFS=$'\t' read -r ahead behind name status; do
        printf '| `%s` | %s | %s | %s |\n' \
            "$(ref_cell "$name")" "$ahead" "$behind" "$(status_md "$status")"
    done
}
