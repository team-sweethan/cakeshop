...
ALTER TABLE `notifications` DROP FOREIGN KEY `fk_notifications_chat_message`;

-- 2. notifications 테이블 신규 컬럼 보충 및 신규 FK 적용
ALTER TABLE `notifications`
    ...
    ADD CONSTRAINT `fk_notifications_receiver_cascade` FOREIGN KEY (`receiver_id`) REFERENCES `members` (`id`) ON DELETE CASCADE,
    ADD CONSTRAINT `fk_notifications_actor`            FOREIGN KEY (`actor_id`)    REFERENCES `members` (`id`) ON DELETE SET NULL,
    ADD CONSTRAINT `fk_notifications_order_setnull`    FOREIGN KEY (`order_id`)    REFERENCES `orders` (`id`)  ON DELETE SET NULL;
...
-- 3. notification_deliveries 레거시 데이터 안전 보정
...
-- 4. notification_deliveries 기존 FK 안전 제거 및 신규 보충
ALTER TABLE `notification_deliveries` DROP FOREIGN KEY `fk_notification_deliveries_notification`;
...
ALTER TABLE `notification_deliveries`
    ...
    ADD CONSTRAINT `fk_notification_deliveries_notification_cascade` FOREIGN KEY (`notification_id`) REFERENCES `notifications` (`id`) ON DELETE CASCADE,
