...
- **`docs/community/PLAN.md`** — 작업 순서, 진행 상태, 하네스, 위험, 결정 로그. 지금 어느 조각을 하는지 여기서 확인한다. **커뮤니티 테스트를 새로 만들면 하네스 표에 올린다** — `CommunityHarnessDocTests`(H36)가 표와 실제 테스트를 **양방향**으로 대조하므로, 표에 없는 테스트를 두거나 표가 없는 테스트를 가리키면 빌드가 깨진다.
...
프로젝트 전체 규칙은 저장소 루트 `AGENTS.md`가 상위 정본이며, 충돌하면 `AGENTS.md`가 우선한다. **도메인 간 연동은 `AGENTS.md`가 한 문단으로만 적고 정본을 `docs/conventions.md` 15절로 넘긴다** — 커뮤니티는 회원 도메인과 물려 있으므로(아래 절) 그쪽을 읽어야 규칙 본문에 닿는다.
...
5. **테스트를 새로 만들었으면 `PLAN.md` 하네스 표에 올린다.** 하네스가 아니라 평범한 단위 테스트라면 `CommunityHarnessDocTests.UNLISTED_ON_PURPOSE`에 이유와 함께 적는다 — 둘 중 하나를 고르는 것이 H36의 요구이고, 그냥 넘어갈 수는 없다.
...
- **목록 정렬은 `?sort=`의 허용값을 `<choose>`로 매핑한다. `${}`로 잇지 않는다.** 모르는 값은 오류가 아니라 기본 정렬로 떨어뜨린다 — 목록은 공개 화면이다. `id` tiebreaker는 어느 정렬에도 붙인다. **좋아요 정렬(`sort=likes`)은 넣지 않기로 한 결정이다** — 인기글이 대신한다. (6.1, 조각 7a)
...
- **인기글은 배치가 확정한 스냅샷만 읽는다. 요청마다 집계하지 않는다.** 그리고 **이미 확정된 날짜는 다시 집계하지 않는다** — 좋아요 취소·댓글 삭제 뒤에 재실행하면 확정된 순위가 조용히 바뀌고, 그러면 스냅샷이 스냅샷인 이유가 사라진다. 재실행은 실패한 날짜에만 뜻이 있다. (6.9)
- **인기글의 댓글 신호는 건수가 아니라 `COUNT(DISTINCT member_id)`다.** `comments`에는 UNIQUE가 없어서 건수로 세면 계정 하나가 댓글 20개로 좋아요 12명분을 만든다 — 계수 15의 근거가 무너진다. (6.9)
- **7일 창의 끝은 `대상일 + 1일 00:00:00 미만`이다. `23:59:59 이하`로 적지 않는다.** `DATETIME(6)`은 그 뒤로도 값이 있어 자정 직전 1초가 통째로 빠진다. (6.9)
- **활동이 0인 날도 "확정했다"를 기록한다.** 안 남기면 "안 돈 날"과 구분되지 않아 화면이 옛 날짜로 폴백하고, 7일 창 밖의 오래된 글이 무기한 걸린다. 폴백은 사용자에게도 운영자에게도 정상으로 보인다. (6.9)
- **인기글 영역을 싣는 조건(1쪽 + 카테고리 필터 없음)은 화면이 아니라 Service에 있다.** 규칙이라 화면이 늘면 두 벌이 되고, 두 벌이 되는 순간 갈린다. **인기글에는 숫자를 싣지 않는다** — 같은 글이 아래 목록에도 나오고 그쪽은 현재 수치다. (6.9)
