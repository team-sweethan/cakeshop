...
| 테스트 추가·변경 | `docs/testing.md` 관련 절과 `PLAN.md` 하네스 표 |
...
4. 새 테스트는 `PLAN.md` 하네스 표에 등록한다. 일반 단위 테스트라면
   `CommunityHarnessDocTests.UNLISTED_ON_PURPOSE`에 이유를 기록한다.
5. 화면 변경은 해당 화면 명세와 렌더링 검증을 함께 갱신한다.
