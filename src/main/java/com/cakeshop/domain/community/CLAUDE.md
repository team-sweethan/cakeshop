...
- **조회수는 정렬·순위의 근거 데이터다. 참고용 표시가 아니다.** (2026-08-03 정책 변경, 2절·6.1·6.2) 그래서 같은 사람이 같은 날 같은 글을 여러 번 열어도 한 번만 오른다.
...
- **`view_count`는 `post_views`에서 파생된 캐시다.** 증분으로 갱신하는 대신 두 값의 일치를 하네스가 지킨다. 어긋나면 이력에서 재계산해 복구한다. (6.2, H14)
...
- **차단·해제도 `lockPost`로 게시글 행을 먼저 잠근다.** (H17) 조회수·좋아요와 같은 행이라 잠금 순서가 어긋나면 교착이다. `posts` 행에 쓰는 경로를 새로 만들 때마다 이 순서를 따진다.
