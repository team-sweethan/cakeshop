...
**`history/`와 `decisions/`는 기본으로 읽지 않는다** — 끝난 조각의 기록과 결정 배경이라 지금 하는 일을
구속하지 않는다. 위 표의 조건에 걸릴 때만 해당 파일 **하나**를 연다.
...
- **조각이 머지되면 그 조각의 착수 항목·구현 기록·검증 목록을 `docs/community/history/`로 옮긴다.**
  `PLAN.md`에는 상태 한 줄과 기록 파일 링크만 남는다. 옮기지 않으면 `PLAN.md`가 다시 부푼다 —
  2026-08-18에 764줄에서 잘라낸 자리다. **위험과 결정 로그는 옮기지 않는다**(현재 구속이다).
- 한 줄로 재구성이 안 되는 결정이면 `docs/community/decisions/`에 ADR 을 세우고 결정 로그에서 가리킨다.
