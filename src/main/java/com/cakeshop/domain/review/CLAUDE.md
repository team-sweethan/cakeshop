...
- **조각 1의 자격 검증은 주문 도메인 계약을 거친다 — 착수 전에 주환님과 시그니처를 합의한다.** A1 목록과 A2·A3의 검증 1~3번이 `order_items`·`orders`를 직접 JOIN하는 모양으로 적혀 있었는데, `docs/conventions.md` 15절이 다른 도메인 테이블의 직접 JOIN·조회를 금지한다. 상품 쪽(D1)에는 적용한 기준을 주문 쪽에는 적용하지 않은 자리였다. **계약을 리뷰 쪽에서 혼자 정하지 않는다.** 15절대로 시그니처만 먼저 합의하고 구현은 stub으로 진행할 수 있지만, **시그니처가 정해지기 전에는 조각 1을 열지 않는다.** (`SPEC.md` A1·9절, `PLAN.md` 조각 1)
...
- **조각 3은 선행 합의가 둘이다.** 수민님과 **작성자 표시명 계약**(ID 묶음 조회), 시은님과 **상품 상세에 후기를 붙이는 방식**. 후자를 빼먹으면 `GET /products/{id}/reviews`는 동작하는데 **고객이 실제로 가는 상품 상세는 계속 `준비 중`이다** — `ProductController.detail`이 서버 렌더링이고 템플릿에 후기를 가져오는 코드가 없다. (`SPEC.md` B1)
- **조각 5는 관리자 검색 계약**(`writer`·`product`)을 수민·주환님과 합의한 뒤 착수한다. (`SPEC.md` C2)
- `SPEC.md` 9절의 **미정 항목**(관리자 숨김 사유·조치 이력 기록, 이미지 첨부)도 마찬가지다.

## 이 도메인에서 절대 하지 않는 것

**리뷰 매퍼는 `reviews`·`review_replies`·`review_images` 외의 테이블을 JOIN하지 않는다. 예외 없다.** 표시용이든 검색용이든 같다. 필요한 값은 그 도메인의 전용 QueryService가 DTO로 돌려준다. (`SPEC.md` 2.7)

- **커뮤니티가 `members`를 직접 JOIN한다고 해서 따라 하지 않는다**(`CommunityMapper.xml` 4곳, `CommunityAdminMapper.xml` 4곳). 리뷰는 더 엄격한 규칙을 쓰기로 정했다 — 표시용을 열어 두면 어디까지가 표시용인지 매번 판단해야 하고, 그 판단이 느슨해지는 순간 자격 검증 같은 업무 규칙이 같은 문으로 들어온다. **실제로 A1·A2·A3가 그렇게 들어와 있었다.**
- **목록은 건별 조회가 아니라 ID 묶음 조회다.** 후기 20건에 회원 조회를 20번 하면 N+1이다. 계약이 `List<Long>`을 받는 모양이어야 한다.
- **제외·검색 조건을 리뷰가 뒤에서 거르지 않는다.** 계약이 먼저 페이지를 자르고 리뷰가 걸러 내면 **화면의 건수와 실제 건수가 갈린다.** A1(작성 완료 제외)과 C2(작성자·상품명 검색)가 같은 자리다. 조건을 계약에 넘겨 ID 목록이나 페이지 결과를 받는다.
- 계약이 없으면 **만들지 말고 합의부터 한다.** 조각별 선행 조건은 `PLAN.md`에, 목록은 `SPEC.md` 2.7·9절에 있다.
...
- **A4(수정)의 UPDATE에는 소유권과 `status = 'PUBLISHED'`를 함께 넣는다.** `UPDATE reviews SET ... WHERE id = ? AND member_id = ? AND status = 'PUBLISHED'` 후 `affectedRows`로 판정. 2.1의 조건부 UPDATE는 **상태 전이**를 두고 쓴 규칙이라 평점·본문 수정은 그 밖에 있다. 앞에서 `PUBLISHED`를 검증만 해 두면 관리자 숨김이 그사이 커밋될 때 **작성자가 숨겨진 후기의 내용을 덮어쓰고**, 해제하는 날 관리자가 차단했던 것과 다른 글이 공개된다. (`SPEC.md` A4)
...
- **B1은 상품이 고객에게 공개되는지 먼저 확인한다**(`ProductQueryService.getSalesInfo`). 없으면 판매 중지 상품의 후기가 계속 공개된다 — 상품 상세는 `p.status = 'ACTIVE'`로 가려지는데 후기 목록은 후기의 `status`만 보기 때문이다. **화면 안에서는 상세가 먼저 404라 드러나지 않고 주소를 직접 넣는 경로에서만 보인다.** **그 계약의 `NOT_ON_SALE` 400을 그대로 흘리지 않고 404로 바꾼다** — 없는 상품(404)과 판매 중지 상품(400)이 다른 응답이면 주소를 훑어 둘을 구분할 수 있어 가리려던 것이 그대로 드러난다. (`SPEC.md` B1)
...
- **평점 범위는 화면·서버·DB 세 겹으로 막는다.** 화면은 별 5개만 렌더링해 0·6에 해당하는 입력을 애초에 만들지 않고, 서버는 `@NotNull` + `@Min(1)` `@Max(5)`, DB는 조각 0의 `CHECK`다. **기본 선택으로 별 하나를 찍어 두지 않는다** — `overall_rating`은 상품 평균으로 나가는 대표값이라 고객이 손대지 않은 값이 집계에 섞인다. 빈 채로 제출하면 돌려보낸다. (`SPEC.md` 2.2)
...
- **집계 SELECT에는 `FOR UPDATE`를 붙인다. 순서를 지키는 것만으로는 부족하다.** MariaDB 기본 `REPEATABLE READ`의 읽기 스냅샷은 **트랜잭션의 첫 평범한 SELECT에서 굳는데**, A3는 잠금을 잡기 전에 A2의 검증 4가지를 평범한 SELECT로 돌린다. 그래서 잠금을 기다리던 트랜잭션은 잠금을 얻은 뒤에도 먼저 커밋된 후기를 보지 못하고 **자기 INSERT만 반영된 값으로 `products`를 덮어쓴다** — `review_count`가 실제보다 작아진다. 잠금 읽기는 스냅샷이 아니라 최신 커밋을 읽는다. 위 잠금 순서와 **같은 뿌리이지만 순서만으로는 닫히지 않는 절반**이고, 이것도 동시 요청 테스트로만 드러난다. (`SPEC.md` D1)
...
- **답글은 `PUBLISHED` 후기에만 단다.** 관리자 화면은 모든 상태를 보여 주므로 검증을 빠뜨리면 숨긴 후기에도 답글이 저장되고 알림까지 나가는데, **그 답글은 고객 화면에서 가려져 있다** — 알림을 받고 들어와도 볼 것이 없다. `DELETED`는 404, `BLOCKED`는 403. **확인과 INSERT를 갈라 놓지 않는다** — 별개 문장이면 답글 작성과 C4 숨김이 동시에 들어올 때 숨김이 먼저 커밋돼도 답글 쪽은 먼저 읽은 `PUBLISHED`를 그대로 쓰고, FK는 후기의 **존재**만 보므로 `BLOCKED` 후기에 답글과 알림이 남는다. 후기 행을 `FOR UPDATE`로 잡고 확인하거나 조건을 저장 문장에 넣고 `affectedRows`로 판정한다(2.1과 같은 모양). (`SPEC.md` C5)
- **조각 6은 관리자 답글 작성만이 아니라 고객 화면의 답글 노출(B4)까지다.** B1·B3에 답글을 붙이지 않으면 관리자는 답글을 쓰고 알림까지 나가는데 **고객이 들어올 화면에 답글이 없다.** (`SPEC.md` B4, `PLAN.md` 조각 6)
