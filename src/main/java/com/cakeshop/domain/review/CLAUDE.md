...
- **`docs/review/HARNESS.md`** — 자동 검증 체계. "이 규칙은 무엇이 지키나"와 각 검사가 **보증하지 않는 것**. 조각이 하네스를 추가하면 여기에 올린다.
...
6. **그 조각이 추가한 하네스를 `docs/review/HARNESS.md` 표에 올리고 `PLAN.md` 조각 표의 상태를 바꾼다.** 하네스 표를 `PLAN.md`에 적으면 H4가 막고, `PLAN.md`가 부르는 번호가 `HARNESS.md`에 없으면 H6이 막는다.
...
- **머지된 Flyway migration은 주석 한 글자도 고치지 않는다.** 체크섬이 바뀌어 이미 반영된 DB에서 검증이 실패한다. `V20260806_075114`의 주석이 지금은 없어진 `docs/review/SPEC.md`를 가리키는데, 그래서 그대로 둔다(`docs/review/HARNESS.md`).
