...
리뷰에서 이 규칙이 걸리는 자리는 둘이다 — 조각 2가 `domain/product/`에 `ProductRatingService`·`ProductReviewMapper`를 만들고, 조각 3이 시은님 `ProductController`에 모델 주입을 넣는다. 두 경우 모두 `담당자`는 **시은**이다.
