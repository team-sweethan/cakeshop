 ...
 * 설명 : 알림 도메인에 만료 임박(3일 이내) 유효 쿠폰 목록을 제공하는 공개 계약 Service이다.
 * ******************************
 */
@Service
...
public class CouponNotificationQueryService {
    ...
    /**
     * 지정된 일수(days) 이내에 만료 예정인 유효 쿠폰 목록을 조회한다.
     */
    public List<CouponNotificationView> findExpiringMemberCouponsWithinDays(int days, int limit) {
        int targetDays = days > 0 ? days : 3;
        int fetchLimit = limit > 0 ? limit : 100;
        return couponNotificationMapper.findExpiringMemberCouponsWithinDays(targetDays, fetchLimit);
    }
}
