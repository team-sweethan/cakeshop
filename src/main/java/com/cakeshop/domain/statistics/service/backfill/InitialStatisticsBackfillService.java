...
import com.cakeshop.domain.statistics.service.aggregation.StatisticsAggregationTransactionService;
...
public class InitialStatisticsBackfillService {
    ...
    public StatisticsBackfillResult backfillInitialStatistics() {
        if (batchRunMapper.findLatestSuccessfulBackfillStartedAt() != null) {
            log.info("초기 통계 백필을 건너뜁니다. 성공한 백필이 이미 존재합니다.");
            return StatisticsBackfillResult.ALREADY_COMPLETED;
        }
        ...
        try {
            ...
            return StatisticsBackfillResult.SKIPPED_RUNNING;
        }
        if (batchRunId == null) {
            log.info("초기 통계 백필을 건너뜁니다. 성공한 백필이 이미 존재합니다.");
            return StatisticsBackfillResult.ALREADY_COMPLETED;
        ...
        try {
            ...
            return StatisticsBackfillResult.SUCCEEDED;
